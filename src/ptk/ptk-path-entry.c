...
/*  this interferes with entering URLs in path bar
    ...
    if ( strcmp( path, actual_path ) )
    {
        ...
        g_signal_handlers_block_matched( G_OBJECT( entry ), G_SIGNAL_MATCH_FUNC, 0, 0, NULL, on_changed, NULL );
        gtk_entry_set_text( GTK_ENTRY( entry ), actual_path );
        gtk_editable_set_position( (GtkEditable*)entry, -1 );
        g_signal_handlers_unblock_matched( G_OBJECT( entry ), G_SIGNAL_MATCH_FUNC, 0, 0, NULL, on_changed, NULL );
    }
    g_free( actual_path );
*/
...
*/
    ...
    g_signal_connect_after( G_OBJECT( gtk_entry_get_buffer( GTK_ENTRY( entry ) ) ), "inserted-text", G_CALLBACK( on_entry_insert ), NULL );
