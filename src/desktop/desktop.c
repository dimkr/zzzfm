...
void fm_turn_on_desktop_icons(gboolean transparent) {
    ...
    theme_change_notify = g_signal_connect( gtk_icon_theme_get_default(), "changed", G_CALLBACK(on_icon_theme_changed), NULL );
    ...
    for ( i = 0; i < n_screens; i++ )
    {
        ...
        // move desktop window to 0,0 so background in window aligns with root window background for transparent panel
        // re     /issues/248
