...
static gboolean  sysfs_file_exists (const char *dir, const char *attribute) {
  gboolean result;
  char *filename;
      ...
      if (p != NULL)
        {
          ...
          if (strcmp (subsystem, "scsi") == 0)
          ...
          else if (strcmp (subsystem, "mmc") == 0)
            {
               ...
               * to map these into names for setting the manufacturer of the drive, e.g. Panasonic, Sandisk etc.
               */
...
{
  const char *udev_property;
  const char *media_name;    //  howdy bub.   How comprehensive (or not) is this enumeration in 2021?
} media_mapping[] =
  {
...
void info_drive_properties ( device_t *device ) {
    ...
    // revision
    device->drive_revision = g_strdup( udev_device_get_property_value( device->udevice, "ID_REVISION" ) );
...
void info_device_properties( device_t *device ) {
    ...
    if (g_strcmp0 (partition_scheme, "mbr") == 0 && (partition_type == 0x05 ||  partition_type == 0x0f ||  partition_type == 0x85))
    {
    } else {
  ...
  for (n = 0; lines[n] != NULL; n++)
    {
      ...
      if (sscanf (lines[n],
        ...
        /* Temporary work-around for btrfs, see       /issues/165
...
//printf("finding %d:%d\n", devmount->major, devmount->minor );
            found = g_list_find_custom( devmounts, (gconstpointer)devmount, (GCompareFunc)cmp_devmounts );
            if ( found )
            {
//printf("    found\n");
...
static gboolean cb_out_watch( GIOChannel *channel, GIOCondition cond, char *data ) {
    gchar *line;
    gsize  size;
    char* parameter;
    char* value;
...
void vfs_volume_monitor_start() {
    ...
    g_spawn_command_line_sync( "/usr/bin/udisks --poll-for-media /dev/sr0", &stdout, &stderr, NULL, NULL );
    if ( stdout )
        g_free( stdout );
    if ( stderr )
        g_free( stderr );
    ...
    if ( mode == HANDLER_MODE_FS )
    {
        ...
        */
        ...
        if ( strstr( command, "%a" ) )
        {
            ...
            {
                // create mount point
                char* point_dir = ptk_location_view_create_mount_point( HANDLER_MODE_FS, vol, NULL, NULL );
                str = command;
    ...
    {   // also used for DEVICE_TYPE_OTHER unmount and prop
        ...
        */
        ...
        if ( strstr( command, "%a" ) )
        {
            ...
            {
                // create mount point
                char* point_dir = ptk_location_view_create_mount_point( HANDLER_MODE_NET, NULL, netmount, NULL );
                str = command;
                command = replace_string( command, "%a", point_dir, FALSE );
                g_free( str );
                if ( mount_point )
        ...
        if ( netmount )
        {
            ...
            char* urlq = bash_quote( action != HANDLER_MOUNT &&  vol && vol->is_mounted ?  vol->udi : netmount->url );
            ...
            // mtab fs type (fuse.ssh)
            char* mtabfsq = bash_quote( action != HANDLER_MOUNT &&  vol && vol->is_mounted ? vol->fs_type : NULL );
            // urlq and mtaburlq will both be the same mtab url if mounted
            char* mtaburlq = bash_quote( action != HANDLER_MOUNT &&  vol && vol->is_mounted ?  vol->device_file : NULL );
            str = command;
...
char* vfs_volume_device_mount_cmd( VFSVolume* vol, const char* options, gboolean* run_in_terminal ) {
    ...
    *run_in_terminal = FALSE;
    command = vfs_volume_handler_cmd( HANDLER_MODE_FS, HANDLER_MOUNT, vol, options, NULL, run_in_terminal, NULL );
    if ( !command )
    {
        // discovery
...
char* vfs_volume_device_unmount_cmd( VFSVolume* vol, gboolean* run_in_terminal ) {
    ...
    *run_in_terminal = FALSE;
    ...
    else if ( vol->device_type == DEVICE_TYPE_OTHER &&  mtab_fstype_is_handled_by_protocol( vol->fs_type ) )
    {
        ...
        command = vfs_volume_handler_cmd( HANDLER_MODE_FS, HANDLER_UNMOUNT, vol, NULL, NULL, run_in_terminal, NULL );
    if ( !command )
    {
        // discovery
...
char* vfs_volume_get_mount_command( VFSVolume* vol, char* default_options, gboolean* run_in_terminal )
{
...
void exec_task( const char* command, gboolean run_in_terminal ) {   // run command as async task with optional terminal
            ...
            else
            {
                if ( xset_get_b( "dev_auto_open" ) )
                {
                    ...
                    if ( main_window )
                    {
                        ...
                        //if ( file_browser )
                        //    ptk_file_browser_emit_open( file_browser, vol->mount_point, PTK_OPEN_DIR ); //PTK_OPEN_NEW_TAB
...
static void vfs_volume_device_added( VFSVolume* volume, gboolean automount ) {           //frees volume if needed
    GList* l;
...
gboolean vfs_volume_init() {
    ...
    if ( !udev )
    {
        printf( "zzzfm: unable to initialize udev\n" );
        return TRUE;
    }
    ...
    if ( !umonitor )
    {
        printf( "zzzfm: cannot create udev monitor\n" );
        goto finish_;
    }
    if ( udev_monitor_enable_receiving( umonitor ) )
    {
        printf( "zzzfm: cannot enable udev monitor receiving\n");
        goto finish_;
    }
    if ( udev_monitor_filter_add_match_subsystem_devtype( umonitor, "block", NULL ) )
    {
        printf( "zzzfm: cannot set udev filter\n");
        goto finish_;
    }
    ...
    if ( ufd == 0 )
    {
        printf( "zzzfm: cannot get udev monitor socket file descriptor\n");
        goto finish_;
    }
    ...
    if ( mchannel != NULL )
    {
        ...
        free_devmounts();
        printf( "zzzfm: error monitoring %s: %s\n", MOUNTINFO, error->message );
        g_error_free (error);
    }
...
gboolean vfs_volume_dir_avoid_changes( const char* dir ) {
    ...
    // dir (eg nfs stat calls block when a write is in progress so file change detection is unwanted)
