...
GdkPixbuf* vfs_load_icon( GtkIconTheme* theme, const char* icon_name, int size ) {
    ...
    GtkIconInfo* inf = gtk_icon_theme_lookup_icon( theme, icon_name, size,   GTK_ICON_LOOKUP_USE_BUILTIN | GTK_ICON_LOOKUP_FORCE_SIZE );
...
gboolean vfs_sudo_cmd_async( const char* cwd, const char* cmd, GError** err ) {
    ...
    ret = g_spawn_async( cwd, argv, NULL,   G_SPAWN_STDOUT_TO_DEV_NULL | G_SPAWN_STDERR_TO_DEV_NULL,   NULL, NULL, NULL, err );
    return ret;
}
