...
*/
static void get_total_size_of_dir( VFSFileTask* task, const char* path, off_t* size, struct stat64* have_stat );
...
{   // returns NULL if all names used; otherwise newly allocated string
    ...
    while ( n && lstat64( new_dest_file, &dest_stat ) == 0 )
    {
        ...
        new_name = g_strdup_printf( "%s-%s%d%s%s", base_name, _("copy"), ++n, ext && ext[0] ? "." : "", ext ? ext : "" );
...
*/
static
gboolean check_overwrite( VFSFileTask* task, const gchar* dest_file, gboolean* dest_exists, char** new_dest_file )
{
    struct stat64 dest_stat;
        ...
        *dest_exists = !lstat64( dest_file, &dest_stat );
        ...
        do
        {
            ...
            *dest_exists = TRUE;
            ...
            // query user
            if ( !task->state_cb( task, VFS_FILE_TASK_QUERY_OVERWRITE, &new_dest, task->state_cb_data ) )
            ...
            if ( task->overwrite_mode != VFS_FILE_TASK_RENAME )
            {
                g_free( new_dest );
                new_dest = NULL;
                if( task->overwrite_mode == VFS_FILE_TASK_OVERWRITE ||  task->overwrite_mode == VFS_FILE_TASK_OVERWRITE_ALL )
                {
                    *dest_exists = !lstat64( dest_file, &dest_stat );
                    if ( !g_strcmp0( task->current_file, task->current_dest ) )
                    {
...
static int vfs_file_task_do_move ( VFSFileTask* task, const char* src_file, const char* dest_file ) {  //MOD void to int
...
static void  vfs_file_task_move( char* src_file, VFSFileTask* task )
{
    ...
    if ( lstat64( src_file, &src_stat ) == 0   && stat64( task->dest_dir, &dest_stat ) == 0 )
    {
        ...
        /*
        else if ( S_ISDIR( src_stat.st_mode ) &&    g_file_test( dest_file, G_FILE_TEST_IS_DIR) )
        {
            // moving a directory onto a directory that exists
        }
        */
...
*/
    ...
    {
        //gtk_text_buffer_get_iter_at_mark( task->exec_err_buf, &iter, task->exec_mark_end );
...
//task->exec_keep_tmp = TRUE;
    ...
    if ( task->exec_terminal )
    {
        ...
        if ( !( terminal && terminal[0] == '/' ) )
        {
            ...
            // do not use xset_msg_dialog if non-main thread
            //vfs_file_task_exec_error( task, 0, str );
            xset_msg_dialog( parent, GTK_MESSAGE_ERROR, _("Terminal Not Available"), NULL, 0, str, NULL, NULL );
            goto _exit_with_error_lean;
        }
    ...
    if ( !task->exec_direct )
    {
        ...
        // build - header
        result = fprintf( file, "#!%s\n#\n# Temporary zzzFM exec script - it is safe to delete this file\n\n", BASHPATH );
        ...
        if ( task->exec_write_root && geteuid() != 0 )
        {
            ...
            {
                char* root_set_path= g_strdup_printf( "%s/zzzfm/%s-as-root", SYSCONFDIR, this_user );
                write_root_settings( file, root_set_path );
                g_free( root_set_path );
                //g_free( this_user );  DON'T
            } else {
                char* root_set_path= g_strdup_printf( "%s/zzzfm/%d-as-root", SYSCONFDIR, geteuid() );
                write_root_settings( file, root_set_path );
                g_free( root_set_path );
            }
        }
         ...
         * http://www.linuxjournal.com/content/start-and-control-konsole-dbus            howdy
         *
         * gnome-terminal removed --disable-factory option as of 3.10      /issues/428
        */
        ...
        {
            if ( geteuid() == 0 ||  ( task->exec_as_user && !strcmp( task->exec_as_user, "root" ) ) )
                result = fprintf( file, "\necho\necho -n '%s: '\nread s", "[ Finished ]  Press Enter to close" );
            else
            {
    ...
    if ( terminal )
    {
        ...
        else if ( strstr( terminal, "sakura" ) )
        {
             ...
             * argument to -e.  zzzFM uses zzzfm-auth to run commands,
    ...
    if ( task->exec_sync )
    {
        result = g_spawn_async_with_pipes( task->dest_dir, argv, NULL, G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL, &pid, NULL, &out, &err, NULL );
    } else {
        result = g_spawn_async_with_pipes( task->dest_dir, argv, NULL, G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL, &pid, NULL, NULL, NULL, NULL );
    }
    ...
    if( !result )
    {
        ...
        str = g_strdup_printf( _("Error executing '%s'\nSee stdout (run zzzfm in a terminal) for debug info"), first_arg );
        g_free( first_arg );
...
//VFSFileTask* task = (VFSFileTask*)ptr;
    ...
    if ( task->recursive )
    {
        // start timer to limit the amount of time to spend on this - can be VERY slow for network filesystems
...
void vfs_file_task_set_chmod( VFSFileTask* task, guchar* chmod_actions )
{
    ...
    memcpy( ( void* ) task->chmod_actions, ( void* ) chmod_actions, sizeof( guchar ) * N_CHMOD_ACTIONS );
}
...
void vfs_file_task_run ( VFSFileTask* task )
{
    if ( task->type != VFS_FILE_TASK_EXEC )
    {
        ...
        task->thread = g_thread_create( ( GThreadFunc ) vfs_file_task_thread, task, TRUE, NULL );
        //task->thread = g_thread_create_full( ( GThreadFunc ) vfs_file_task_thread, task, 0, TRUE, TRUE, G_THREAD_PRIORITY_NORMAL, NULL );
        //pthread_t tid;
...
void vfs_file_task_free ( VFSFileTask* task )
{
    ...
    if ( task->chmod_actions )
        g_slice_free1( sizeof( guchar ) * N_CHMOD_ACTIONS, task->chmod_actions );
...
* NOTE: *size should be set to zero before calling this function.
*/
void get_total_size_of_dir( VFSFileTask* task, const char* path, off64_t* size, struct stat64* have_stat )
{
    GDir * dir;
    const char* name;
    char* full_path;
...
void vfs_file_task_set_progress_callback( VFSFileTask* task, VFSFileTaskProgressCallback cb, gpointer user_data )
{
    task->progress_cb = cb;
    task->progress_cb_data = user_data;
}

void vfs_file_task_set_state_callback( VFSFileTask* task, VFSFileTaskStateCallback cb, gpointer user_data )
{
    task->state_cb = cb;
