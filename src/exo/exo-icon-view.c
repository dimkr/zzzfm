...
struct _ExoIconViewItem {
   ...
   * before, after are used to calculate the cell area relative to the box.
...
exo_icon_view_get_type (void)
{
    ...
    {
        type = _exo_g_type_register_simple (GTK_TYPE_CONTAINER, "ExoIconView",
                                            sizeof (ExoIconViewClass), exo_icon_view_class_init,
                                            sizeof (ExoIconView), exo_icon_view_init);
...
#if GTK_CHECK_VERSION (3, 0, 0)
static void  exo_icon_view_get_preferred_width (GtkWidget *widget, gint      *minimal_width, gint      *natural_width)
{
    GtkRequisition requisition;
...
static void  exo_icon_view_get_preferred_height (GtkWidget *widget, gint      *minimal_height, gint      *natural_height)
{
    GtkRequisition requisition;
...
static void  exo_icon_view_class_init (ExoIconViewClass *klass)
{
   ...
   **/
    g_object_class_install_property (gobject_class, PROP_MARKUP_COLUMN,
   ...
   * display of pixbufs.
   **/
    g_object_class_install_property (gobject_class, PROP_PIXBUF_COLUMN,
   ...
   **/
    g_object_class_install_property (gobject_class, PROP_TEXT_COLUMN,
...
static void  exo_icon_view_get_property (GObject      *object, guint         prop_id, GValue       *value, GParamSpec   *pspec)
{
...
static void  exo_icon_view_set_property (GObject      *object, guint         prop_id, const GValue *value, GParamSpec   *pspec)
{
    ExoIconView *icon_view = EXO_ICON_VIEW (object);
...
static void  exo_icon_view_realize (GtkWidget *widget)
{
...
#if !GTK_CHECK_VERSION (3, 0, 0)
    /* GTK2: Attach style/background - this breaks 'dark theme version' styles in GTK3     /issues/578    */
     ...
     * /issues/627  shows that this correct method of setting the style causes a slowdown (??), so going back to the
     * broken method */
    ...
    gtk_style_context_add_class (gtk_widget_get_style_context (GTK_WIDGET (widget)), GTK_STYLE_CLASS_VIEW);
#endif
...
#if GTK_CHECK_VERSION (3, 0, 0)
static gboolean  exo_icon_view_draw (GtkWidget    *widget, cairo_t      *cr)
#else
static gboolean  exo_icon_view_expose_event (GtkWidget      *widget, GdkEventExpose *event)
#endif
{
    ExoIconViewDropPosition dest_pos;
    ExoIconViewPrivate     *priv = EXO_ICON_VIEW (widget)->priv;
   ...
   */
    ...
    if (G_UNLIKELY (priv->scroll_to_path != NULL))
    {
        ...
        {
            /* try to scroll again */
            exo_icon_view_scroll_to_path (icon_view, path, priv->scroll_to_use_align, priv->scroll_to_row_align, priv->scroll_to_col_align);
    ...
    if (G_UNLIKELY (dest_item != NULL))
    {
...
#if GTK_CHECK_VERSION (3, 0, 0)
            gtk_paint_focus (gtk_widget_get_style(widget), cr, gtk_widget_get_state (widget), widget,
                 #else
            gtk_paint_focus (gtk_widget_get_style(widget), priv->bin_window, gtk_widget_get_state (widget), NULL, widget,
                 #endif
                             "iconview-drop-indicator", dest_item->area.x, dest_item->area.y, dest_item->area.width, dest_item->area.height);
            break;
        ...
        case EXO_ICON_VIEW_DROP_ABOVE:
            ...
            gtk_paint_focus (gtk_widget_get_style(widget), priv->bin_window, gtk_widget_get_state (widget), NULL, widget,
                 #endif
                             "iconview-drop-indicator", dest_item->area.x, dest_item->area.y - 1, dest_item->area.width, 2);
            break;
        ...
        case EXO_ICON_VIEW_DROP_LEFT:
            ...
            gtk_paint_focus (gtk_widget_get_style(widget), priv->bin_window, gtk_widget_get_state (widget), NULL, widget,
                 #endif
                             "iconview-drop-indicator", dest_item->area.x - 1, dest_item->area.y, 2, dest_item->area.height);
            break;
        ...
        case EXO_ICON_VIEW_DROP_BELOW:
            ...
            gtk_paint_focus (gtk_widget_get_style(widget), priv->bin_window, gtk_widget_get_state (widget), NULL, widget,
                 #endif
                             "iconview-drop-indicator", dest_item->area.x, dest_item->area.y + dest_item->area.height - 1, dest_item->area.width, 2);
            break;
        ...
        case EXO_ICON_VIEW_DROP_RIGHT:
            ...
            gtk_paint_focus (gtk_widget_get_style(widget), priv->bin_window, gtk_widget_get_state (widget), NULL, widget,
                 #endif
                             "iconview-drop-indicator", dest_item->area.x + dest_item->area.width - 1, dest_item->area.y, 2, dest_item->area.height);
...
static gboolean  rubberband_scroll_timeout (gpointer user_data)
{
...
static gboolean  exo_icon_view_motion_notify_event (GtkWidget      *widget, GdkEventMotion *event)
{
    ExoIconViewItem *item;
    GtkAllocation    allocation;
    ExoIconView     *icon_view = EXO_ICON_VIEW (widget);
...
static void  exo_icon_view_forall (GtkContainer *container, gboolean      include_internals, GtkCallback   callback, gpointer      callback_data)
{
    ExoIconView *icon_view = EXO_ICON_VIEW (container);
    GList       *lp;
...
static void  exo_icon_view_item_activate_cell (ExoIconView         *icon_view, ExoIconViewItem     *item, ExoIconViewCellInfo *info, GdkEvent            *event)
{
...
static void  exo_icon_view_put (ExoIconView     *icon_view, GtkWidget       *widget, ExoIconViewItem *item, gint             cell)
{
    ExoIconViewChild *child;
...
static void  exo_icon_view_start_editing (ExoIconView         *icon_view, ExoIconViewItem     *item, ExoIconViewCellInfo *info, GdkEvent            *event)
{
    GtkCellRendererMode mode;
    GtkCellEditable    *editable;
    GdkRectangle        cell_area;
...
static void  exo_icon_view_stop_editing (ExoIconView *icon_view, gboolean     cancel_editing)
{
    ExoIconViewItem *item;
...
static gboolean  exo_icon_view_button_press_event (GtkWidget      *widget, GdkEventButton *event)
{
...
static gboolean  exo_icon_view_button_release_event (GtkWidget      *widget, GdkEventButton *event)
{
    ExoIconViewItem *item;
    ExoIconView     *icon_view = EXO_ICON_VIEW (widget);
    GtkTreePath     *path;
...
static gboolean  exo_icon_view_scroll_event (GtkWidget      *widget, GdkEventScroll *event)
{
    GtkAdjustment *adjustment;
    ExoIconView   *icon_view = EXO_ICON_VIEW (widget);
    gdouble        delta;
...
static gboolean  exo_icon_view_key_press_event (GtkWidget   *widget, GdkEventKey *event)
{
    ExoIconView *icon_view = EXO_ICON_VIEW (widget);
    GdkScreen   *screen;
    GdkEvent    *new_event;
    ...
    gtk_window_move (GTK_WINDOW (icon_view->priv->search_window), gdk_screen_get_width (screen) + 1, gdk_screen_get_height (screen) + 1);
   ...
   */
    ...
    /* we check to make sure that the entry tried to handle the,  and that the text has actually changed.   */
...
static gboolean  exo_icon_view_focus_out_event (GtkWidget     *widget, GdkEventFocus *event)
{
    ExoIconView *icon_view = EXO_ICON_VIEW (widget);
...
static gboolean  exo_icon_view_leave_notify_event (GtkWidget        *widget, GdkEventCrossing *event)
{
...
static void  exo_icon_view_start_rubberbanding (ExoIconView  *icon_view, gint          x, gint          y)
{
    ...
    {
        g_signal_handlers_block_matched (G_OBJECT (icon_view), G_SIGNAL_MATCH_DATA, 0, 0, NULL, NULL, drag_data);
    }
}
...
static void  exo_icon_view_stop_rubberbanding (ExoIconView *icon_view)
{
    ...
    if (G_LIKELY (icon_view->priv->doing_rubberband))
    {
        ...
        if (G_LIKELY (drag_data != NULL))
        {
            g_signal_handlers_unblock_matched (G_OBJECT (icon_view), G_SIGNAL_MATCH_DATA, 0, 0, NULL, NULL, drag_data);
        }
    }
}
...
static gboolean  exo_icon_view_item_hit_test (ExoIconView      *icon_view, ExoIconViewItem  *item, gint              x, gint              y, gint              width, gint              height)
{
    GList *l;
    GdkRectangle box;
...
#if GTK_CHECK_VERSION (3, 0, 0)
static void  exo_icon_view_set_hadjustment (ExoIconView   *icon_view, GtkAdjustment *hadj)
{
    ...
    g_signal_connect (icon_view->priv->hadjustment, "value-changed", G_CALLBACK (exo_icon_view_adjustment_changed), icon_view);

    if (icon_view->priv->vadjustment)
        exo_icon_view_adjustment_changed (NULL, icon_view);
}

static void  exo_icon_view_set_vadjustment (ExoIconView   *icon_view, GtkAdjustment *vadj)
{
    ...
    g_signal_connect (icon_view->priv->vadjustment, "value-changed", G_CALLBACK (exo_icon_view_adjustment_changed), icon_view);

    if (icon_view->priv->hadjustment)
        exo_icon_view_adjustment_changed (NULL, icon_view);
}
#else
static void  exo_icon_view_set_adjustments (ExoIconView   *icon_view, GtkAdjustment *hadj, GtkAdjustment *vadj)
{
...
static void  exo_icon_view_set_adjustment_upper (GtkAdjustment *adj, gdouble        upper)
{
                           ...
                           gint         max_rows)
{
    ...
    *x = icon_view->priv->margin;
    ...
    do
    {
        icons = exo_icon_view_layout_single_col (icon_view, icons, item_height, col, x, maximum_height, max_rows);
...
static gint
exo_icon_view_layout_rows (ExoIconView *icon_view, gint         item_width,
                           gint        *y,
                           gint        *maximum_width,
                           gint         max_cols)
{
...
static void  exo_icon_view_layout (ExoIconView *icon_view)
{
    ...
    if (gtk_widget_get_realized (GTK_WIDGET (icon_view)))
    {
        gdk_window_resize (priv->bin_window, MAX (priv->width, allocation.width), MAX (priv->height, allocation.height));
    }
...
static void  exo_icon_view_calculate_item_size (ExoIconView     *icon_view, ExoIconViewItem *item)
{
    ...
    for (lp = icon_view->priv->cell_list; lp != NULL; lp = lp->next)
    {
        ...
        gtk_cell_renderer_get_size (info->cell, GTK_WIDGET (icon_view), NULL, NULL, NULL, &item->box[info->position].width,
                          ...
                          gboolean         draw_focus)
{
    ...
    gdk_draw_rectangle (drawable, GTK_WIDGET (icon_view)->style->black_gc, FALSE, x, y, item->area.width, item->area.height);
#endif
...
#if GTK_CHECK_VERSION (3, 0, 0)
        gtk_cell_renderer_render (info->cell, cr, GTK_WIDGET (icon_view), &cell_area, &cell_area, flags);
#else
        gtk_cell_renderer_render (info->cell, drawable, GTK_WIDGET (icon_view), &cell_area, &cell_area, area, flags);
#endif
...
static gboolean  layout_callback (gpointer user_data)
{
    ExoIconView *icon_view = EXO_ICON_VIEW (user_data);
...
static void  layout_destroy (gpointer user_data)
{
...
static void  exo_icon_view_set_cursor_item (ExoIconView     *icon_view, ExoIconViewItem *item, gint             cursor_cell)
{
...
exo_icon_view_get_item_at_coords (const ExoIconView    *icon_view, gint                  x, gint                  y, gboolean              only_in_cell, ExoIconViewCellInfo **cell_at_pos)
{
    const ExoIconViewPrivate *priv = icon_view->priv;
    ExoIconViewCellInfo      *info;
    ExoIconViewItem          *item;
...
static void  exo_icon_view_row_changed (GtkTreeModel *model,
                           GtkTreePath  *path,
                           GtkTreeIter  *iter,
                           ExoIconView  *icon_view)
{
...
static void  exo_icon_view_row_inserted (GtkTreeModel *model, GtkTreePath  *path, GtkTreeIter  *iter, ExoIconView  *icon_view)
{
    ExoIconViewItem *item;
    gint             index;
...
static void  exo_icon_view_row_deleted (GtkTreeModel *model, GtkTreePath  *path, ExoIconView  *icon_view)
{
    ExoIconViewItem *item;
    gboolean         changed = FALSE;
    GList           *list;
...
static void  exo_icon_view_rows_reordered (GtkTreeModel *model, GtkTreePath  *parent, GtkTreeIter  *iter, gint         *new_order, ExoIconView  *icon_view)
{
    GList **list_array;
    GList  *list;
    gint   *order;
...
static void  exo_icon_view_add_move_binding (GtkBindingSet  *binding_set, guint           keyval, guint           modmask, GtkMovementStep step, gint            count)
{
...
static gboolean  exo_icon_view_real_move_cursor (ExoIconView     *icon_view, GtkMovementStep  step, gint             count)
{
...
static ExoIconViewItem *
find_item_page_up_down (ExoIconView     *icon_view, ExoIconViewItem *current, gint             count)
{
...
static gboolean  exo_icon_view_select_all_between (ExoIconView     *icon_view, ExoIconViewItem *anchor, ExoIconViewItem *cursor)
{
    GList *items;
...
static void  exo_icon_view_move_cursor_up_down (ExoIconView *icon_view, gint         count)
{
    ExoIconViewItem *item;
    gboolean         dirty = FALSE;
    GList           *list;
    ...
    {
        dirty = exo_icon_view_unselect_all_internal (icon_view);
        dirty = exo_icon_view_select_all_between (icon_view, icon_view->priv->anchor_item, item) || dirty;
    }
...
static void  exo_icon_view_move_cursor_page_up_down (ExoIconView *icon_view, gint         count)
{
    ExoIconViewItem *item;
    gboolean dirty = FALSE;
...
static void  exo_icon_view_move_cursor_left_right (ExoIconView *icon_view, gint         count)
{
    ExoIconViewItem *item;
    gboolean         dirty = FALSE;
    GList           *list;
...
static void  exo_icon_view_move_cursor_start_end (ExoIconView *icon_view, gint         count)
{
    ExoIconViewItem *item;
    gboolean         dirty = FALSE;
    GList           *lp;
 ...
 */
static void  exo_icon_view_get_item_needed_size (ExoIconView     *icon_view, ExoIconViewItem *item, gint            *width, gint            *height)
{
    GList               *lp;
    ExoIconViewCellInfo *info;
...
exo_icon_view_get_cell_info (ExoIconView     *icon_view, GtkCellRenderer *renderer)
{
    GList *lp;
...
static void  exo_icon_view_cell_layout_pack_start (GtkCellLayout   *layout, GtkCellRenderer *renderer, gboolean         expand)
{
    ExoIconViewCellInfo *info;
...
static void  exo_icon_view_cell_layout_pack_end (GtkCellLayout   *layout, GtkCellRenderer *renderer, gboolean         expand)
{
    ExoIconViewCellInfo *info;
    ExoIconView         *icon_view = EXO_ICON_VIEW (layout);
...
static void  exo_icon_view_cell_layout_add_attribute (GtkCellLayout   *layout, GtkCellRenderer *renderer, const gchar     *attribute, gint             column)
{
    ExoIconViewCellInfo *info;
...
static void  exo_icon_view_cell_layout_set_cell_data_func (GtkCellLayout         *layout, GtkCellRenderer       *cell, GtkCellLayoutDataFunc  func, gpointer               func_data, GDestroyNotify         destroy)
{
    ExoIconViewCellInfo *info;
...
static void  exo_icon_view_cell_layout_clear_attributes (GtkCellLayout   *layout, GtkCellRenderer *renderer)
{
    ExoIconViewCellInfo *info;
...
static void  exo_icon_view_cell_layout_reorder (GtkCellLayout   *layout, GtkCellRenderer *cell, gint             position)
{
    ExoIconViewCellInfo *info;
    ExoIconView         *icon_view = EXO_ICON_VIEW (layout);
    GList               *lp;
 ...
 **/
GtkTreePath*
exo_icon_view_get_path_at_pos (const ExoIconView *icon_view, gint               x, gint               y)
{
    ExoIconViewItem *item;
...
static void  update_text_cell (ExoIconView *icon_view)
{
            ...
            icon_view->priv->markup_column == -1)
    {
        ...
        if (icon_view->priv->markup_column != -1)
            gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (icon_view), info->cell, "markup", icon_view->priv->markup_column, NULL);
        else
            gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (icon_view), info->cell, "text", icon_view->priv->text_column, NULL);
    }
}
...
static void  update_pixbuf_cell (ExoIconView *icon_view)
{
    ...
    if (icon_view->priv->pixbuf_column == -1)
    {
        ...
        gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (icon_view), info->cell, "pixbuf", icon_view->priv->pixbuf_column, NULL);
    }
}
...
{
    g_object_set_data (G_OBJECT (context), I_("exo-icon-view-status-pending"), GINT_TO_POINTER (suggested_action));
}
              ...
              gboolean        drop_append_mode)
{
    ...
    g_object_set_data_full (G_OBJECT (context), I_("exo-icon-view-dest-row"), dr, (GDestroyNotify) dest_row_free);
}
...
static gboolean  set_destination (ExoIconView    *icon_view,
                 GdkDragContext *context,
                 gint            x,
                 gint            y,
...
static GtkTreePath*
get_logical_destination (ExoIconView *icon_view, gboolean    *drop_append_mode)
{
...
static gboolean  exo_icon_view_maybe_begin_drag (ExoIconView *icon_view, GdkEventMotion *event)
{
    ...
    if (!gtk_drag_check_threshold (GTK_WIDGET (icon_view), icon_view->priv->press_start_x, icon_view->priv->press_start_y, event->x, event->y))
        goto out;
    ...
    path = exo_icon_view_get_path_at_pos (icon_view, icon_view->priv->press_start_x, icon_view->priv->press_start_y);
...
/* Target side drag signals */
static void  exo_icon_view_drag_leave (GtkWidget      *widget, GdkDragContext *context, guint           time)
{
    ExoIconView *icon_view;
                                  ...
                                  guint             time)
{
    ...
    if (suggested_action)
    {
       ...
       * supposed to call drag_status, not actually paste in the data.
       */
        path = get_logical_destination (icon_view, &drop_append_mode);
...
exo_icon_view_create_drag_icon (ExoIconView *icon_view, GtkTreePath *path)
{
    ...
    for (lp = icon_view->priv->items; lp != NULL; lp = lp->next)
    {
        ...
        {
#if GTK_CHECK_VERSION (3, 0, 0)
            drawable = cairo_image_surface_create (CAIRO_FORMAT_ARGB32, item->area.width + 2, item->area.height + 2);

            cr = cairo_create (drawable);
#else
            drawable = gdk_pixmap_new (icon_view->priv->bin_window, item->area.width + 2, item->area.height + 2, -1);
...
static void  exo_icon_view_search_move (GtkWidget  *widget, ExoIconView *icon_view, gboolean     move_up)
{
    ...
    if (G_LIKELY (retval))
    {
        ...
        {
            count = 0;
            exo_icon_view_search_iter (icon_view, model, &iter, text, &count, icon_view->priv->search_selected_iter);
        }
    }
}
...
#if GTK_CHECK_VERSION (3, 0, 0)
static void exo_icon_view_search_preedit_changed (GtkEntry *entry, gchar *preedit,
                                      #else
static void exo_icon_view_search_preedit_changed (GtkIMContext *im_context,
                                      #endif
                                      ExoIconView  *icon_view)
...
static gboolean  exo_icon_view_search_start (ExoIconView *icon_view, gboolean     keybinding)
{
    GTypeClass *klass;
   ...
   * we don't want to start interactive search if one of our children has the focus.
   */
    if (!gtk_widget_has_focus (GTK_WIDGET (icon_view)))
        return FALSE;
