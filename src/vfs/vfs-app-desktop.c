...
// If file_name is not a full path, this function searches default paths for the desktop file.
VFSAppDesktop* vfs_app_desktop_new( const char* file_name ) {
    ...
    if( load )
    {
        ...
        app->comment = g_key_file_get_locale_string ( file, desktop_entry_name, "Comment", NULL, NULL);
...
gboolean vfs_app_desktop_rename( char* desktop_file_path, char* new_name ) {
    ...
    // load
    if ( !g_key_file_load_from_file( kfile, desktop_file_path, G_KEY_FILE_KEEP_COMMENTS
...
* file_list is a null-terminated file list containing full paths of the files passed to app.
...
gboolean vfs_app_desktop_open_files( GdkScreen* screen, const char* working_dir, VFSAppDesktop* app, GList* file_paths, GError** err ) {
    char* exec = NULL;
    char* cmd;
    GList* l;
    gchar** argv = NULL;
        ...
        if( vfs_app_desktop_open_multiple_files( app ) )
        {
            ...
            if ( cmd )
            {
                if ( vfs_app_desktop_open_in_terminal( app ) )
                    exec_in_terminal( sn_desc, app->path && app->path[0] ? app->path : working_dir, cmd );
                else
                {
                    /* g_debug( "Execute %s\n", cmd ); */
