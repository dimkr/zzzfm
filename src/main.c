...
char* get_inode_tag() {
    ...
    return g_strdup_printf( "%d=%d:%d-%ld", getuid(), major( stat_buf.st_dev ), minor( stat_buf.st_dev ), stat_buf.st_ino );
}
...
gboolean single_instance_check() {
    ...
    {
        fprintf( stderr, "zzzfm: socket init failure\n" );
        ret = 1;
        goto _exit;
    }
...
int send_socket_command( int argc, char* argv[], char** reply ) {
    ...
    *reply = NULL;
    if ( argc < 3 )
    {
        fprintf( stderr, _("zzzfm: --socket-cmd requires an argument\n") );
        return 1;
    }
    ...
    if ( ( sock = socket( AF_UNIX, SOCK_STREAM, 0 ) ) == -1 )
    {
        fprintf( stderr, _("zzzfm: could not create socket\n") );
        return 1;
    }
    ...
    {
        fprintf( stderr, _("zzzfm: could not connect to socket (not running? or DISPLAY not set?)\n") );
        return 1;
    }
    ...
    if ( sock_reply->len != 0 )
    {
        *reply = g_strdup( sock_reply->str + 1 );
        ...
        fprintf( stderr, _("zzzfm: invalid response from socket\n") );
        ret = 1;
    }
...
void show_socket_help() {
    ...
    printf( "\n%s\n", _("METHODS\n-------") );
    printf( "zzzfm -s set [OPTIONS] PROPERTY [VALUE...]\n" );
    printf( "    %s\n", _("Sets a property") );

    printf( "\nzzzfm -s get [OPTIONS] PROPERTY\n" );
    printf( "    %s\n", _("Gets a property") );

    printf( "\nzzzfm -s set-task [OPTIONS] TASKID TASKPROPERTY [VALUE...]\n" );
    printf( "    %s\n", _("Sets a task property") );

    printf( "\nzzzfm -s get-task [OPTIONS] TASKID TASKPROPERTY\n" );
    printf( "    %s\n", _("Gets a task property") );

    printf( "\nzzzfm -s run-task [OPTIONS] TASKTYPE ARGUMENTS\n" );
    printf( "    %s\n", _("Starts a new task") );

    printf( "\nzzzfm -s emit-key [OPTIONS] KEYCODE [MODIFIER]\n" );
    printf( "    %s\n", _("Activates a menu item by emitting its shortcut key") );

    printf( "\nzzzfm -s activate [OPTIONS] NAME\n" );
    printf( "    %s\n", _("Runs custom command or shows submenu named NAME") );

    printf( "\nzzzfm -s add-event EVENT COMMAND...\n" );
    printf( "    %s\n", _("Add asynchronous handler COMMAND to EVENT") );

    printf( "\nzzzfm -s replace-event EVENT COMMAND...\n" );
    printf( "    %s\n", _("Add synchronous handler COMMAND to EVENT, replacing default handler") );

    printf( "\nzzzfm -s remove-event EVENT COMMAND...\n" );
    ...
    printf( "\n--window WINDOWID\n" );
    printf( "    %s zzzfm -s set --window 0x104ca80 window_size 800x600\n", _("Specify window.  eg:") );
    printf( "--panel PANEL\n" );
    printf( "    %s zzzfm -s set --panel 2 bookmarks_visible true\n", _("Specify panel 1-4.  eg:") );
    printf( "--tab TAB\n" );
    printf( "    %s zzzfm -s set --tab 3 selected_filenames fstab\n", _("Specify tab 1-...  eg:") );
  ...
  //printf( "web URL                         %s\n", _("Open URL in user's web browser") );    // howdy bub    why-oh-why use Filemanager to open?!?
    ...
    printf( "    window_size=\"$(zzzfm -s get window_size)\"\n" );
    printf( "    zzzfm -s set window_size 1024x768\n" );
    printf( "    zzzfm -s set column_width name 100\n" );
    printf( "    zzzfm -s set-task $fm_my_task progress 25\n" );
    printf( "    zzzfm -s run-task --window $fm_my_window cmd --task --popup ls /etc\n" );
    printf( "    zzzfm -s run-task copy --dir /etc fstab hosts /destdir\n" );
    printf( "    zzzfm -r /etc; sleep 0.3; zzzfm -s set selected_filenames fstab hosts\n" );
    printf( "    zzzfm -s set clipboard_copy_files /etc/fstab /etc/hosts\n" );
    printf( "    zzzfm -s emit-key 0xffbe 0   # press F1 to show Help\n" );
    printf( "    zzzfm -s activate --window $fm_my_window \"Custom Menu\"\n" );
    printf( "    zzzfm -s add-event evt_pnl_sel 'zzzfm -s set statusbar_text \"$fm_file\"'\n\n" );

    printf( "    #!%s\n", BASHPATH );
    printf( "    eval copied_files=\"$(zzzfm -s get clipboard_copy_files)\"\n" );
...
static void open_in_tab( FMMainWindow** main_window, const char* real_path ) {
    ...
    if( G_UNLIKELY( !*main_window ) )
    {
        ...
        if( G_UNLIKELY( ! daemon_mode ) )
            init_folder();                      // this fn can also called from 3 other points
...
#else
        show_pref = 1;
        fprintf( stderr, "zzzfm: %s\n", _("This build of zzzFM has desktop integration disabled") );
#endif
    }
...
int main ( int argc, char *argv[] ) {
    ...
    {
        char* bash_error = g_strdup_printf( _( "zzzFM requires bash to be installed.  Other shells are NOT equivalent as zzzFM uses features only found in genuine bash (v4+).\n\nThe program %s was not found.  Install bash or use configure option --with-bash-path to specify a custom location at build time." ), BASHPATH );
        fprintf( stderr, "zzzfm: %s\n", bash_error );
    ...
    /* ensure that there is only one instance of zzzfm.
...
/*
    if( run && xset_get_b( "main_save_exit" ) )
    {
        ...
        if ( err_msg )
            printf( "zzzfm: Error: Unable to save session\n       %s\n", err_msg );
    }
*/
    vfs_volume_finalize();
    vfs_mime_type_clean();
