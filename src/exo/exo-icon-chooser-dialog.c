...
static gboolean  exo_icon_chooser_dialog_visible_func (GtkTreeModel *model, GtkTreeIter  *iter, gpointer      user_data)
{
...
static void  exo_icon_chooser_dialog_combo_changed (GtkWidget            *combo, ExoIconChooserDialog *icon_chooser_dialog)
{
    ExoIconChooserDialogPrivate *priv = EXO_ICON_CHOOSER_DIALOG_GET_PRIVATE (icon_chooser_dialog);
    ExoIconChooserContext        context;
    GtkTreeModel                *model;
...
#if GTK_CHECK_VERSION (2, 16, 0)
    gtk_entry_set_icon_sensitive (GTK_ENTRY (priv->filter_entry), GTK_ENTRY_ICON_SECONDARY, !exo_str_is_empty (text));
#endif
...
#if GTK_CHECK_VERSION (2, 16, 0)
static void  exo_icon_chooser_dialog_entry_clear (GtkEntry             *entry, GtkEntryIconPosition  icon_pos, GdkEvent             *event)
{
...
static gboolean  exo_icon_chooser_dialog_button_press_event (ExoIconChooserDialog *icon_chooser_dialog, GdkEvent             *event, gpointer             *user_data)
{
    // On double-click, accept the selected icon
    if (event->type == GDK_2BUTTON_PRESS)
        gtk_dialog_response(GTK_DIALOG (icon_chooser_dialog), GTK_RESPONSE_ACCEPT);
