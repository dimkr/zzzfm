...
static gboolean hit_test_text( DesktopWindow* self, int x, int y, DesktopItem** next_item );
...
//    self->sort_type = GTK_SORT_ASCENDING;
    ...
    metrics = pango_context_get_metrics( pc, gtk_widget_get_style( ((GtkWidget*)self) )->font_desc, pango_context_get_language(pc));
...
#endif
{
    ...
    if( G_UNLIKELY( ! gtk_widget_get_visible (w) || ! gtk_widget_get_mapped (w) ) )
        return TRUE;
/*
    gdk_draw_drawable( w->window, self->gc, self->background, evt->area.x, evt->area.y, evt->area.x, evt->area.y, evt->area.width, evt->area.height );
*/
/*
    gdk_gc_set_tile( self->gc, self->background );
    gdk_gc_set_fill( self->gc,  GDK_TILED );/*
    gdk_draw_rectangle( w->window, self->gc, TRUE, evt->area.x, evt->area.y, evt->area.width, evt->area.height );
*/
...
#if !GTK_CHECK_VERSION (3, 0, 0)
        gdk_colormap_alloc_color ( gtk_widget_get_colormap( (GtkWidget*)win ), &win->bg, FALSE, TRUE );
#endif
...
// Allocating colors is unnecessary with gtk3
#if !GTK_CHECK_VERSION (3, 0, 0)
            gdk_colormap_alloc_color ( gtk_widget_get_colormap( (GtkWidget*)win ), &win->fg, FALSE, TRUE );
#endif
        }
        if( shadow )
        {
...
// Allocating colors is unnecessary with gtk3
#if !GTK_CHECK_VERSION (3, 0, 0)
            gdk_colormap_alloc_color ( gtk_widget_get_colormap( (GtkWidget*)win ), &win->shadow, FALSE, TRUE );
#endif
        }
        if( gtk_widget_get_visible( (GtkWidget*)win ) )
            gtk_widget_queue_draw(  (GtkWidget*)win );
    ...
    if ( !win->transparent )
    {
        ...
        if( pixmap )
        {
            ...
            XChangeProperty( xdisplay, xroot,
                        gdk_x11_get_xatom_by_name("_XROOTPMAP_ID"), XA_PIXMAP, 32, PropModeReplace, (guchar *) &xpixmap, 1);

            XSetWindowBackgroundPixmap( xdisplay, xroot, xpixmap );
        } else {
            //sfm 1.0.6 added this section [Teklad]
            // https://github.com/Teklad/zzzfm-ng/commit/ac19c081210fd845af191f128791ebf9ab607a71
            XColor col;
...
void paint_rubber_banding_rect( DesktopWindow* self ) {
    ...
    if( rect.width <= 0 || rect.height <= 0 )
        return;
/*
    gtk_widget_style_get( GTK_WIDGET(self), "selection-box-color", &clr, "selection-box-alpha", &alpha, NULL);
*/
    ...
    if( self->bg_type == DW_BG_TILE )
    {
        ...
        pix = gdk_pixbuf_get_from_drawable( NULL, self->background, gdk_drawable_get_colormap(self->background), 0, 0, 0, 0, pattern_w, pattern_h );
        */
    }
    else if( self->bg_type != DW_BG_COLOR )
    {
        if( self->background )
#if GTK_CHECK_VERSION (3, 0, 0)
            pix = gdk_pixbuf_get_from_surface( self->surface, rect.x, rect.y, rect.width, rect.height );
#else
            pix = gdk_pixbuf_get_from_drawable( NULL, self->background, gdk_drawable_get_colormap(self->background), rect.x, rect.y, 0, 0, rect.width, rect.height );
#endif
    }
    ...
    if( pix )
    {
        ...
        if( self->bg_type == DW_BG_TILE ) /* this is currently unreachable */
        {
            ...
            if( pattern )
            {
                gdk_draw_pixbuf( pattern, gc, pix, 0, 0, 0, 0, pattern_w, pattern_h, GDK_RGB_DITHER_NONE, 0, 0 );
                gdk_gc_set_tile( gc, pattern );
                gdk_gc_set_fill( gc, GDK_TILED );
        gdk_draw_rectangle( gtk_widget_get_window( ((GtkWidget*)self) ), gc, TRUE, rect.x, rect.y, rect.width-1, rect.height-1 );
...
gboolean is_mouse_pressed(GdkEvent* evt) {
     ...
     * https://github.com/Teklad/zzzfm-ng/commit/9d0012f7c1dd6f09e099cac92dd149d4418245a8
     */
...
gboolean on_mouse_move( GtkWidget* w, GdkEventMotion* evt ) {
    ...
    else if( self->rubber_bending )
    {
               ...
               )
        {
            ...
            gtk_drag_begin( w, target_list, GDK_ACTION_COPY|GDK_ACTION_MOVE|GDK_ACTION_LINK, 1, (GdkEvent*)evt );
...
void on_drag_data_get( GtkWidget* w, GdkDragContext* ctx, GtkSelectionData* data, guint info, guint time ) {
    ...
    if( info == DRAG_TARGET_URI_LIST )
    {
        ...
        if( uri_list )
        {
            gtk_selection_data_set( data, text_uri_list_atom, 8, (guchar *)uri_list, len );
            g_free (uri_list);
        }
    }
...
void move_desktop_items( DesktopWindow* self, GdkDragContext* ctx, DesktopItem* target_item ) {
    ...
    //sel_items = g_list_sort_with_data( sel_items, //                                (GCompareDataFunc)comp_item_by_name, NULL );
...
void on_drag_data_received( GtkWidget* w, GdkDragContext* ctx, gint x, gint y, GtkSelectionData* data, guint info, guint time ) {
    ...
    if( gtk_selection_data_get_target( data ) == text_uri_list_atom )
    {
        ...
        if ( files )
        {
            ...
            task = ptk_file_task_new( file_action, file_list, dest_dir ? dest_dir : vfs_get_desktop_dir(), GTK_WINDOW( self ), NULL );
            // get insertion box
            if ( self->sort_by == DW_SORT_CUSTOM )
            {
                if ( !item )
                ...
                ptk_file_task_set_complete_notify( task, (GFunc)desktop_window_insert_task_complete, self );
            }
...
void desktop_window_rename_selected_files( DesktopWindow* self, GList* files, const char* cwd ) {
        ...
        if ( vfs_file_info_is_desktop_entry( file ) )
        {
            ...
            if ( filename )
            {
                ...
                if ( !have_rw_access( target ) )
                {
                    ...
                    g_free( target );
                    if ( !xset_text_dialog( GTK_WIDGET( self ), _("Change Desktop Item Name"), NULL, FALSE, msg, NULL, name, &new_name, NULL, FALSE, NULL ) || !new_name )
                    {
                        g_free( msg );
                        g_free( path );
                        g_free( filename );
    ...
    if ( self->sort_by == DW_SORT_CUSTOM )
    {
        ...
        g_timeout_add_seconds( 2, ( GSourceFunc ) on_rename_item_invalidate, self );
    }
}
...
_key_found:
            ...
            else if ( g_str_has_prefix( set->name, "paste_" ) )
            {
                ...
                if ( !strcmp( xname, "link" ) )
                {
                    ...
                    ptk_clipboard_paste_links( GTK_WINDOW( desktop ), vfs_get_desktop_dir(), NULL, (GFunc)desktop_window_insert_task_complete, GTK_WINDOW( desktop ) );
                }
                else if ( !strcmp( xname, "target" ) )
                {
                    desktop_window_set_insert_item( desktop );
                    ptk_clipboard_paste_targets( GTK_WINDOW( desktop ), vfs_get_desktop_dir(), NULL, (GFunc)desktop_window_insert_task_complete, GTK_WINDOW( desktop ) );
                }
                else if ( !strcmp( xname, "as" ) )
                {
                    desktop_window_set_insert_item( desktop );
                    ptk_file_misc_paste_as( desktop, NULL, vfs_get_desktop_dir(), (GFunc)desktop_window_insert_task_complete );
                }
            }
...
void on_realize( GtkWidget* w ) {
    ...
    if ( self->transparent && !g_strcmp0( wmname, "Compiz" ) )
    {
        ...
        gdk_window_set_type_hint( gtk_widget_get_window(w), GDK_WINDOW_TYPE_HINT_DESKTOP );
    }
...
#define WIN_HINTS_SKIP_FOCUS      (1<<0)    /* skip "alt-tab" */
    ...
    XChangeProperty(gdk_x11_get_default_xdisplay(), GDK_WINDOW_XID(gtk_widget_get_window(w)), XInternAtom(gdk_x11_get_default_xdisplay(), "_WIN_HINTS", False), XA_CARDINAL, 32,
...
gboolean on_scroll( GtkWidget *w, GdkEventScroll *evt ) {
    if ( ((DesktopWindow*)w)->transparent )
    {
        ...
        if ( !g_strcmp0( wmname, "Compiz" ) )
        {
             ...
             * the root window, because in that case, zzzFM doesn't work as a
             * desktop-type window. */
            forward_event_to_rootwin( gtk_widget_get_screen( w ), ( GdkEvent* ) evt );
            return TRUE;
        }
    ...
    if ( ao->path && g_file_test( ao->path, G_FILE_TEST_EXISTS ) )
    {
        ...
        if ( ao->open_file )
        {
            ...
            if ( g_file_test( ao->path, G_FILE_TEST_IS_DIR ) )
            {
                ...
                ptk_open_files_with_app( cwd, sel_files, NULL, self, NULL, FALSE, TRUE );
...
void layout_items( DesktopWindow* self ) {
    ...
    pango_layout_set_width( self->pl, MAX( self->label_w, self->icon_size ) *  PANGO_SCALE );  // 100
...
void on_file_listed( VFSDir* dir, gboolean is_cancelled, DesktopWindow* self ) {
    ...
    for( l = dir->file_list; l; l = l->next )
    {
        ...
        if ( self->sort_by == DW_SORT_CUSTOM )
        {
            if ( order_hash && ( ptr = g_hash_table_lookup( order_hash, vfs_file_info_get_name( fi ) ) ) )
            {
    ...
    else
    {
        ...
        if ( unordered )
        {
            unordered = g_list_sort_with_data( unordered, (GCompareDataFunc)comp_item_by_name, self );
...
void on_file_created( VFSDir* dir, VFSFileInfo* file, gpointer user_data ) {
    ...
    GCompareDataFunc comp_func = get_sort_func( self );
    if ( comp_func )
        self->items = g_list_insert_sorted_with_data( self->items, item, get_sort_func(self), self );
    else
    {
        // custom sort
        GList* ll;
...
void desktop_window_copycmd( DesktopWindow* desktop, GList* sel_files, char* cwd, char* setname ) {
    ...
    if ( !strcmp( setname, "copy_loc" ) || !strcmp( setname, "move_loc" ) ||  ( !copy_dest && !move_dest ) )
    {
        ...
        if ( set2->desc )
            ...
            folder = cwd;
        path = xset_file_dialog( GTK_WIDGET( desktop ), GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER, _("Choose Location"), folder, NULL );
    ...
    if ( copy_dest || move_dest )
    {
        ...
        {
            xset_msg_dialog( GTK_WIDGET( desktop ), GTK_MESSAGE_ERROR, _("Invalid Destination"), NULL, 0, _("Destination same as source"), NULL, NULL );
            g_free( dest_dir );
            return;
        }
        ...
        // task
        PtkFileTask* task = ptk_file_task_new( file_action, file_list, dest_dir, GTK_WINDOW( desktop ), NULL );
        ptk_file_task_run( task );
        g_free( dest_dir );
    } else {
...
void paint_item( DesktopWindow* self, DesktopItem* item, GdkRectangle* expose_area ) {
...
#if GTK_CHECK_VERSION (3, 0, 0)
    gtk_cell_renderer_render( self->icon_render, cr, widget, &item->icon_rect, &item->icon_rect, state );
#else
    gtk_cell_renderer_render( self->icon_render, drawable, widget, &item->icon_rect, &item->icon_rect, expose_area, state );
#endif
    ...
    if ( vfs_file_info_is_symlink( item->fi ) && link_icon )
    {
        ...
        gdk_cairo_set_source_pixbuf ( cr, link_icon, item->icon_rect.x, item->icon_rect.y );
    ...
    {
#if GTK_CHECK_VERSION (3, 0, 0)
        gtk_paint_focus( gtk_widget_get_style(widget), cr, GTK_STATE_NORMAL,/*item->is_selected ? GTK_STATE_SELECTED : GTK_STATE_NORMAL,*/
                        widget, "icon_view", item->text_rect.x, item->text_rect.y, item->text_rect.width, item->text_rect.height);
#else
    ...
    if ( sort_by == DW_SORT_CUSTOM )
    {
        ...
        if ( order_hash )
        {
            ...
            for ( l = win->items; l; l = l->next )
            {
                ...
                if ( ptr = g_hash_table_lookup( order_hash, vfs_file_info_get_name( item->fi ) ) )
                {
                    order = GPOINTER_TO_INT( ptr ) - 1;
                    ll = g_list_nth( items, order );
                    if ( !ll )
            ...
            if ( unordered )
            {
                unordered = g_list_sort_with_data( unordered, (GCompareDataFunc)comp_item_by_name, win );
                items = g_list_concat( items, unordered );
            }
...
void desktop_window_add_application( DesktopWindow* desktop ) {
    ...
    if ( sel_files )
    {
        ...
        {
            char* full_path = g_build_filename( desktop->dir->path, vfs_file_info_get_name( (VFSFileInfo*)sel_files->data ), NULL );
            if ( full_path )
                vfs_file_info_reload_mime_type( (VFSFileInfo*)sel_files->data, full_path );
            g_free( full_path );
        }
    ...
    app = (char *) ptk_choose_app_for_mime_type( GTK_WINDOW( desktop ), mime_type, TRUE, TRUE, FALSE, FALSE );
    if ( app )
    {
        ...
        if ( path && g_file_test( path, G_FILE_TEST_IS_REGULAR ) )
        {
            ...
            PtkFileTask* task = ptk_file_task_new( VFS_FILE_TASK_LINK, sel_files, vfs_get_desktop_dir(), GTK_WINDOW( desktop ), NULL );
            ptk_file_task_run( task );
        }
        else
            g_free( path );
...
static void custom_order_write( DesktopWindow* self ) {
    ...
    if ( file )
    {
        ...
        {
            if ( ((DesktopItem*)l->data)->fi )
                fprintf( file, "%d=%s\n", i, vfs_file_info_get_name( ((DesktopItem*)l->data)->fi ) );
            i++;
        }
        fclose( file );
    }
...
static GHashTable* custom_order_read( DesktopWindow* self ) {
    ...
    if ( file )
    {
        order_hash = g_hash_table_new_full( g_str_hash, g_str_equal, (GDestroyNotify)g_free, NULL );
        while ( fgets( line, sizeof( line ), file ) )
        {
            ...
            if ( sep = strchr( line, '=' ) )
            {
                ...
                else if ( !strchr( line, '_' ) )  // forward compat for attribs
                {
                    ...
                    if ( order < 1 )
                        continue;
                    g_hash_table_insert( order_hash, (gpointer)g_strdup( sep + 1 ), GINT_TO_POINTER( order ) );
                }
            }
        }
        fclose( file );
...
static GdkFilterReturn on_rootwin_event ( GdkXEvent *xevent, GdkEvent *event, gpointer data ) {
    ...
    if ( evt->type == PropertyNotify )
    {
        if( evt->atom == ATOM_NET_WORKAREA )
        {
            ...
            /* This doesn't seem to have the desired effect, and also desktop window size should be based on screen size not WA    /issues/300
...
void forward_event_to_rootwin( GdkScreen *gscreen, GdkEvent *event ) {
    ...
    XSendEvent( dpy, xev.window, False, ButtonPressMask | ButtonReleaseMask, ( XEvent * ) & xev );
    if ( xev2.type == 0 )
        return ;
