...
static void remove_actions( const char* type, GArray* actions ) {   //sfm 0.7.7+ added
    char** removed = NULL;
    gsize n_removed = 0, r;
    int i;
...
static char* make_custom_desktop_file( const char* desktop_id, const char* mime_type ) {
    ...
    if( G_LIKELY( g_str_has_suffix(desktop_id, ".desktop") ) )
    {
        ...
        if( G_UNLIKELY( ! name || ! g_key_file_load_from_file( kf, name, G_KEY_FILE_KEEP_TRANSLATIONS, NULL ) ) )
        {
            g_free( name );
...
//g_print( "    path = %s\n", path );
        ...
        if ( opened )
        {
            ...
            {
                apps = g_key_file_get_string_list( file, groups[k], type, &n_apps, NULL );
                if ( apps )
                {
                    for ( i = 0; i < n_apps; ++i )
                    {
