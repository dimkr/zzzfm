...
enum {
    CONTEXT_COL_DISP, CONTEXT_COL_SUB,
    CONTEXT_COL_COMP,
    CONTEXT_COL_VALUE
};
...
char* context_build( ContextData* ctxt ) {
    ...
    if ( gtk_tree_model_get_iter_first( model, &it ) )
    {
        ...
        do
        {
            gtk_tree_model_get( model, &it, CONTEXT_COL_VALUE, &value, CONTEXT_COL_SUB, &sub, CONTEXT_COL_COMP, &comp, -1 );
...
void enable_context( ContextData* ctxt ) {
    GtkTreeIter it;
    gboolean is_sel = gtk_tree_selection_get_selected( gtk_tree_view_get_selection( GTK_TREE_VIEW( ctxt->view ) ), NULL, NULL );
    gtk_widget_set_sensitive( GTK_WIDGET( ctxt->btn_remove ), is_sel );
    gtk_widget_set_sensitive( GTK_WIDGET( ctxt->btn_apply ), is_sel );
    //gtk_widget_set_sensitive( GTK_WIDGET( ctxt->hbox_match ), gtk_tree_model_get_iter_first(
    ...
    if ( ctxt->context && ctxt->context->valid )
    {
        ...
        if ( rules )
        {
            ...
            else if ( action == CONTEXT_SHOW && gtk_combo_box_get_active( GTK_COMBO_BOX( ctxt->box_action ) ) == CONTEXT_DISABLE )
...
void on_context_button_press( GtkWidget* widget, ContextData* ctxt ) {
    ...
    if ( widget == GTK_WIDGET( ctxt->btn_add ) ||  widget == GTK_WIDGET( ctxt->btn_apply ) )
    {
        ...
        gtk_list_store_set( GTK_LIST_STORE( model ), &it, CONTEXT_COL_DISP, disp, CONTEXT_COL_SUB, sub, CONTEXT_COL_COMP, comp, CONTEXT_COL_VALUE, value, -1 );
        g_free( disp );
        g_free( value );
        gtk_widget_set_sensitive( GTK_WIDGET( ctxt->btn_ok ), TRUE );
        if ( widget == GTK_WIDGET( ctxt->btn_add ) )
            gtk_tree_selection_select_iter( gtk_tree_view_get_selection( GTK_TREE_VIEW( ctxt->view ) ), &it );
        enable_context( ctxt );
        return;
    }
...
void on_context_row_activated( GtkTreeView* view, GtkTreePath* tree_path, GtkTreeViewColumn* col, ContextData* ctxt ) {
    GtkTreeIter it;
    char* value;
    int sub, comp;
...
gboolean on_current_value_button_press( GtkWidget *widget, GdkEventButton *event, ContextData* ctxt ) {
    ...
    {
        gtk_entry_set_text( GTK_ENTRY( gtk_bin_get_child( GTK_BIN( ctxt->box_value ) ) ), gtk_label_get_text( ctxt->current_value ) );
        gtk_widget_grab_focus( ctxt->box_value );
        return TRUE;
    }
    return FALSE;
...
void on_context_entry_insert( GtkEntryBuffer *buf, guint position, gchar *chars, guint n_chars, gpointer user_data ) {   // remove linefeeds from pasted text
...
void enable_options( ContextData* ctxt ) {
    gtk_widget_set_sensitive( ctxt->opt_keep_term, gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( ctxt->opt_terminal ) ) );
    ...
    gtk_widget_set_sensitive( ctxt->cmd_vbox_msg, gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( ctxt->cmd_opt_confirm ) )
                           ||  gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( ctxt->cmd_opt_input ) ) );
/*
// howdy
    gtk_widget_set_sensitive( ctxt->item_icon, !gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( ctxt->cmd_opt_checkbox ) )
...
gboolean is_command_script_newer( ContextData* ctxt ) {
    ...
    {
        if ( statbuf.st_mtime != ctxt->script_stat.st_mtime ||  statbuf.st_size != ctxt->script_stat.st_size )
            return TRUE;
    }
    return FALSE;
}
...
void load_command_script( ContextData* ctxt, XSet* set ) {
    ...
    gtk_text_view_set_editable( GTK_TEXT_VIEW( ctxt->cmd_script ), ( !file || have_access ) );
...
void save_command_script( ContextData* ctxt, gboolean query ) {
    ...
    if ( !gtk_text_buffer_get_modified( buf ) )
        return;
    if ( query && xset_msg_dialog( ctxt->dlg, GTK_MESSAGE_QUESTION, _("Save Modified Script?"), NULL,
                                   GTK_BUTTONS_YES_NO, _("Save your changes to the command script?"), NULL, NULL ) == GTK_RESPONSE_NO )
        return;
    if ( is_command_script_newer( ctxt ) &&  xset_msg_dialog( ctxt->dlg, GTK_MESSAGE_QUESTION, _("Overwrite Script?"), NULL,
                                   GTK_BUTTONS_YES_NO, _("The command script on disk has changed.\n\nDo you want to overwrite it?"), NULL, NULL ) == GTK_RESPONSE_NO )
        return;
...
void on_script_toggled( GtkWidget* item, ContextData* ctxt ) {
    ...
    if ( gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( ctxt->cmd_opt_line ) ) )
    {
        ...
        char* str = g_strdup_printf( "%s  $fm_cmd_dir  %s", _("Command Dir"), dir_has_files( path ) ? "" : _("(no files)") );
...
void on_cmd_opt_toggled( GtkWidget* item, ContextData* ctxt ) {
    enable_options( ctxt );
    if ( ( item == ctxt->cmd_opt_confirm || item == ctxt->cmd_opt_input ) &&  gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( item ) ) )
...
void on_ignore_context_toggled( GtkWidget* item, ContextData* ctxt ) {
    gtk_widget_set_sensitive( ctxt->vbox_context, !gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( item ) ) );
}
...
void on_edit_button_press( GtkWidget* btn, ContextData* ctxt ) {
    ...
    if ( gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( ctxt->cmd_opt_line ) ) )
    {
        ...
        {
            xset_msg_dialog( GTK_WIDGET( ctxt->dlg ), GTK_MESSAGE_ERROR, _("Error"), NULL, 0,
...
void on_open_browser( GtkComboBox* box, ContextData* ctxt ) {
    ...
    {
        // Data Dir
        folder = g_build_filename( xset_get_config_dir(), "mydat", ctxt->set->name, NULL );
...
void on_type_changed( GtkComboBox* box, ContextData* ctxt ) {
    XSet* rset = ctxt->set;
    XSet* mset = xset_get_plugin_mirror( rset );   // howdy flea
    int job = gtk_combo_box_get_active( GTK_COMBO_BOX( box ) );
    if ( job < ITEM_TYPE_COMMAND )
    {
        ...
        if ( job == ITEM_TYPE_BOOKMARK )
        {
            ...
            gtk_button_set_image( GTK_BUTTON( ctxt->item_browse ), xset_get_image( GTK_STOCK_OPEN, GTK_ICON_SIZE_BUTTON ) );
            gtk_label_set_text( GTK_LABEL( ctxt->target_label ), _("Targets:  (a semicolon-separated list of paths or URLs)") );
        } else {
            gtk_widget_show( ctxt->item_choose );
            gtk_widget_show( ctxt->hbox_opener );
            gtk_button_set_image( GTK_BUTTON( ctxt->item_browse ), xset_get_image( GTK_STOCK_EXECUTE, GTK_ICON_SIZE_BUTTON ) );
            gtk_label_set_text( GTK_LABEL( ctxt->target_label ), _("Target:  (a .desktop or executable file)") );
        }
    } else {
        // Command
    ...
    gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( ctxt->opt_terminal ), mset->in_terminal == XSET_B_TRUE  && !ctxt->reset_command );
    gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( ctxt->opt_keep_term ), mset->keep_terminal == XSET_B_TRUE  || ctxt->reset_command );
    gtk_entry_set_text( GTK_ENTRY( ctxt->cmd_user ), rset->y ? rset->y : "" );
    gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( ctxt->opt_task ), mset->task == XSET_B_TRUE  || ctxt->reset_command );
    gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( ctxt->opt_task_pop ), mset->task_pop == XSET_B_TRUE    && !ctxt->reset_command );
    gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( ctxt->opt_task_err ), mset->task_err == XSET_B_TRUE   || ctxt->reset_command );
    gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( ctxt->opt_task_out ), mset->task_out == XSET_B_TRUE   || ctxt->reset_command );
    gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( ctxt->opt_scroll ), mset->scroll_lock != XSET_B_TRUE  || ctxt->reset_command );
...
void on_browse_button_clicked( GtkWidget* widget, ContextData* ctxt ) {
    ...
    {
        // Bookmark Browse
        char* add_path = xset_file_dialog( ctxt->dlg, GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER, _("Choose Folder"), ctxt->context->var[CONTEXT_DIR], NULL );
        if ( add_path && add_path[0] )
        {
            char* old_path = multi_input_get_text( ctxt->item_target );
            char* new_path = g_strdup_printf( "%s%s%s", old_path && old_path[0] ? old_path : "", old_path && old_path[0] ? "; " : "", add_path );
        ...
        {
            // Choose
            VFSMimeType* mime_type = vfs_mime_type_get_from_type( ctxt->context->var[CONTEXT_MIME] &&
                        ctxt->context->var[CONTEXT_MIME][0] ?  ctxt->context->var[CONTEXT_MIME] : XDG_MIME_TYPE_UNKNOWN );
            char* app = (char*)ptk_choose_app_for_mime_type( GTK_WINDOW( gtk_widget_get_toplevel(
                                                    GTK_WIDGET( ctxt->dlg ) ) ), mime_type, TRUE, FALSE, FALSE, FALSE );
            if ( app && app[0] )
            {
                GtkTextBuffer* buf = gtk_text_view_get_buffer( GTK_TEXT_VIEW( ctxt->item_target ) );
            ...
            // Browse
            char* exec_path = xset_file_dialog( ctxt->dlg, GTK_FILE_CHOOSER_ACTION_OPEN, _("Choose Executable"), "/usr/bin", NULL );
...
void replace_item_props( ContextData* ctxt ) {
    ...
    if ( !rset->lock && rset->menu_style != XSET_MENU_SUBMENU &&  rset->menu_style != XSET_MENU_SEP &&  rset->tool <= XSET_TOOL_CUSTOM )
    {
        ...
        if ( x == XSET_CMD_LINE )
        {
            ...
            if ( rset->line && strlen( rset->line ) > 2000 )
                xset_msg_dialog( ctxt->dlg, GTK_MESSAGE_WARNING, _("Command Line Too Long"), NULL, GTK_BUTTONS_OK,
        ...
        mset->in_terminal = gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( ctxt->opt_terminal ) )
                                                && !is_bookmark_or_app ?  XSET_B_TRUE : XSET_B_UNSET;
        mset->keep_terminal = gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( ctxt->opt_keep_term ) )
                                                && !is_bookmark_or_app ?  XSET_B_TRUE : XSET_B_UNSET;
        mset->task = gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( ctxt->opt_task ) )
                                                && !is_bookmark_or_app ?  XSET_B_TRUE : XSET_B_UNSET;
        ...
        mset->task_out = gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( ctxt->opt_task_out ) )
                                                && !is_bookmark_or_app ?  XSET_B_TRUE : XSET_B_UNSET;
        mset->scroll_lock = gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( ctxt->opt_scroll ) )
                                                || is_bookmark_or_app ?  XSET_B_UNSET : XSET_B_TRUE;
    ...
    {
        // name
        if ( rset->lock && rset->in_terminal == XSET_B_UNSET &&  g_strcmp0( rset->menu_label, gtk_entry_get_text( GTK_ENTRY( ctxt->item_name ) ) ) )
            // built-in label has been changed from default, save it
            rset->in_terminal = XSET_B_TRUE;

        g_free( rset->menu_label );
        if ( rset->tool > XSET_TOOL_CUSTOM && !g_strcmp0( gtk_entry_get_text( GTK_ENTRY( ctxt->item_name ) ), xset_get_builtin_toolitem_label( rset->tool ) ) )
    ...
    // Ignore Context
    xset_set_b( "context_dlg", gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( ctxt->ignore_context ) ) );
}
...
void on_prop_notebook_switch_page( GtkNotebook *notebook, GtkWidget *page, guint page_num, ContextData* ctxt ) {
    GtkWidget* widget;
    if ( page_num == 0 )
...
static void on_entry_buffer_inserted_text( GtkEntryBuffer* buf, guint position, gchar* chars, guint n_chars, ContextData* ctxt ) {
    // howdy
    return;
}


static void on_entry_buffer_deleted_text( GtkEntryBuffer* buf, guint position, guint n_chars, ContextData* ctxt ) {
...
void xset_item_prop_dlg( XSetContext* context, XSet* set, int page ) {
    ...
    ctxt->dlg = gtk_dialog_new_with_buttons( set->tool ? _("Toolbar Item Properties") :
                        _("Menu Item Properties"), GTK_WINDOW( ctxt->parent ), GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, NULL, NULL );
    ...
    if ( width && height )
        ...
        gtk_window_set_default_size( GTK_WINDOW( ctxt->dlg ), 600, 600 );   // howdy   ok

    gtk_button_set_focus_on_click( GTK_BUTTON( gtk_dialog_add_button( GTK_DIALOG( ctxt->dlg ), GTK_STOCK_HELP, GTK_RESPONSE_HELP ) ), FALSE );
    gtk_dialog_add_button( GTK_DIALOG( ctxt->dlg ), GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL );
    ctxt->btn_ok = GTK_BUTTON( gtk_dialog_add_button( GTK_DIALOG( ctxt->dlg ), GTK_STOCK_OK, GTK_RESPONSE_OK ) );
    ...
    gtk_box_pack_start( GTK_BOX( gtk_dialog_get_content_area( GTK_DIALOG( ctxt->dlg ) ) ), ctxt->notebook, TRUE, TRUE, 0 );
    ...
    gtk_notebook_append_page( GTK_NOTEBOOK( ctxt->notebook ), align, gtk_label_new_with_mnemonic( set->tool ? _("_Toolbar Item") :  _("_Menu Item") ) );
    ...
    gtk_table_attach( table, ctxt->item_name, 1, 2, row, row + 1, GTK_EXPAND | GTK_FILL, GTK_SHRINK, 0, 0 );
    ...
    gtk_table_attach( table, ctxt->item_key, 1, 2, row, row + 1, GTK_EXPAND | GTK_FILL, GTK_SHRINK, 0, 0 );
...
//   howdy
//      CURRENTLY, THE ABILITY TO CHOOSE A "MENU ITEM CUSTOM ICON" IS SUPPRESSED
//      TESTED ON ANTIX 19FULL, i FOUND THAT 10500 (ten thousand five hundred!) ( cumulative size on disk: 50MB+ )
//      ARE PREINSTALLED... AND, UPON CLICKING THE "CHOOSE" BUTTON... STALLS THE PROGRAM   45SECONDS WAITING FOR
//      ALL OF EM TO BE LOADED INTO THE CHOOSER DIALOG SCROLLED WINDOW.
//      (AND THE TEST SCENARIO WAS LOADING FROM FROM RAMDRIVE!)


    ctxt->item_icon = gtk_entry_new();
    g_signal_connect( G_OBJECT( gtk_entry_get_buffer( GTK_ENTRY( ctxt->item_icon ) ) ), "inserted-text", G_CALLBACK( on_entry_buffer_inserted_text ), ctxt );
    g_signal_connect( G_OBJECT( gtk_entry_get_buffer( GTK_ENTRY( ctxt->item_icon ) ) ), "deleted-text", G_CALLBACK( on_entry_buffer_deleted_text ), ctxt );
    ...
    gtk_button_set_image( GTK_BUTTON( ctxt->item_choose ), xset_get_image( GTK_STOCK_OPEN, GTK_ICON_SIZE_BUTTON ) );
    ...
    //ctxt->show_tool = gtk_check_button_new_with_mnemonic( _("S_how In Toolbar") );
    //gtk_box_pack_start( GTK_BOX( vbox ), GTK_WIDGET( ctxt->show_tool ), FALSE, TRUE, 16 );
    ...
    gtk_container_add ( GTK_CONTAINER ( align ), vbox );
    gtk_notebook_append_page( GTK_NOTEBOOK( ctxt->notebook ), align, gtk_label_new_with_mnemonic( _("Con_text") ) );

    GtkListStore* list = gtk_list_store_new( 4, G_TYPE_STRING, G_TYPE_INT, G_TYPE_INT, G_TYPE_STRING );
    ...
    g_signal_connect( G_OBJECT( ctxt->view ), "row-activated", G_CALLBACK( on_context_row_activated ), ctxt );
    g_signal_connect( G_OBJECT( gtk_tree_view_get_selection( GTK_TREE_VIEW( ctxt->view ) ) ), "changed", G_CALLBACK( on_context_selection_change ), ctxt );
    ...
    gtk_button_set_image( ctxt->btn_remove, xset_get_image( "GTK_STOCK_REMOVE", GTK_ICON_SIZE_BUTTON ) );
    gtk_button_set_focus_on_click( ctxt->btn_remove, FALSE );
    g_signal_connect( G_OBJECT( ctxt->btn_remove ), "clicked", G_CALLBACK( on_context_button_press ), ctxt );
    ...
    g_signal_connect( G_OBJECT( ctxt->btn_apply ), "clicked", G_CALLBACK( on_context_button_press ), ctxt );
    ...
    // see   /issues/43      // this seems to have no effect
    ...
    gtk_combo_box_text_append_text( GTK_COMBO_BOX_TEXT( ctxt->box_match ), _("doesn't match any rule:") );
    gtk_combo_box_text_append_text( GTK_COMBO_BOX_TEXT( ctxt->box_match ), _("doesn't match all rules:") );
    g_signal_connect( G_OBJECT( ctxt->box_match ), "changed", G_CALLBACK( on_context_action_changed ), ctxt );
    ...
    g_signal_connect( G_OBJECT( ctxt->box_action ), "changed", G_CALLBACK( on_context_action_changed ), ctxt );
    ...
    g_signal_connect( G_OBJECT( ctxt->current_value ), "button-press-event", G_CALLBACK( on_current_value_button_press ), ctxt );
    g_signal_connect_after( G_OBJECT( gtk_entry_get_buffer( GTK_ENTRY( gtk_bin_get_child(
                                    GTK_BIN( ctxt->box_value ) ) ) ) ), "inserted-text", G_CALLBACK( on_context_entry_insert ), NULL );
    g_signal_connect( G_OBJECT( GTK_ENTRY( gtk_bin_get_child( GTK_BIN( ctxt->box_value  ) ) ) ), "key-press-event", G_CALLBACK( on_context_entry_keypress ), ctxt );
    ...
    gtk_box_pack_start( GTK_BOX( ctxt->hbox_match ), GTK_WIDGET( gtk_label_new( _("item if context") ) ), FALSE, TRUE, 4 );
...
//                        GTK_WIDGET( label ), FALSE, TRUE, 8 );
    ...
    gtk_box_pack_start( GTK_BOX( ctxt->hbox_opener ), GTK_WIDGET( gtk_label_new( _("If enabled, use as handler for:") ) ), FALSE, TRUE, 0 );
    ...
    // Ignore Context
    ctxt->ignore_context = gtk_check_button_new_with_mnemonic( _("_Ignore Context / Show All  (global setting)") );
    ...
    g_signal_connect( G_OBJECT( ctxt->ignore_context ), "toggled", G_CALLBACK( on_ignore_context_toggled ), ctxt );
    ...
    while ( get_rule_next( &elements, &sub, &comp, &value ) )
    {
        ...
        gtk_list_store_set( GTK_LIST_STORE( list ), &it, CONTEXT_COL_DISP, disp, CONTEXT_COL_SUB, sub, CONTEXT_COL_COMP, comp, CONTEXT_COL_VALUE, value, -1 );
        g_free( disp );
        if ( value )
            g_free( value );
        is_rules = TRUE;
    ...
    gtk_container_add ( GTK_CONTAINER ( align ), vbox );
    gtk_notebook_append_page( GTK_NOTEBOOK( ctxt->notebook ), align, gtk_label_new_with_mnemonic( _("Comm_and") ) );

    hbox = gtk_hbox_new( FALSE, 8 );
    ctxt->cmd_opt_line = gtk_radio_button_new_with_mnemonic( NULL, _("Command _Line") );
    ctxt->cmd_opt_script = gtk_radio_button_new_with_mnemonic_from_widget( GTK_RADIO_BUTTON( ctxt->cmd_opt_line ), _("_Script") );
    ...
    g_signal_connect( G_OBJECT( ctxt->cmd_edit_root ), "clicked", G_CALLBACK( on_edit_button_press ), ctxt );
    ...
    gtk_scrolled_window_set_policy ( GTK_SCROLLED_WINDOW ( ctxt->cmd_scroll_script ), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC );
    gtk_scrolled_window_set_shadow_type( GTK_SCROLLED_WINDOW( ctxt->cmd_scroll_script ), GTK_SHADOW_ETCHED_IN );
    ...
    g_signal_connect_after( G_OBJECT( ctxt->cmd_script ), "populate-popup", G_CALLBACK( on_script_popup ), NULL );
    ...
    vbox = gtk_vbox_new( FALSE, 4 );
    gtk_container_add ( GTK_CONTAINER ( align ), vbox );
    gtk_notebook_append_page( GTK_NOTEBOOK( ctxt->notebook ), align, gtk_label_new_with_mnemonic( _("Optio_ns") ) );
    ...
    gtk_box_pack_start( GTK_BOX( ctxt->opt_hbox_task ), GTK_WIDGET( ctxt->opt_task_pop ), FALSE, TRUE, 0 );
    gtk_box_pack_start( GTK_BOX( ctxt->opt_hbox_task ), GTK_WIDGET( ctxt->opt_task_err ), FALSE, TRUE, 6 );
    gtk_box_pack_start( GTK_BOX( ctxt->opt_hbox_task ), GTK_WIDGET( ctxt->opt_task_out ), FALSE, TRUE, 6 );
    ...
    ctxt->cmd_opt_checkbox = gtk_radio_button_new_with_mnemonic_from_widget( GTK_RADIO_BUTTON( ctxt->cmd_opt_normal ), _("Checkbox") );
    ctxt->cmd_opt_confirm = gtk_radio_button_new_with_mnemonic_from_widget( GTK_RADIO_BUTTON( ctxt->cmd_opt_normal ), _("Confirmation") );
    ctxt->cmd_opt_input = gtk_radio_button_new_with_mnemonic_from_widget( GTK_RADIO_BUTTON( ctxt->cmd_opt_normal ), _("Input") );
    ...
    gtk_scrolled_window_set_policy ( GTK_SCROLLED_WINDOW ( ctxt->cmd_scroll_msg ), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC );
    gtk_scrolled_window_set_shadow_type( GTK_SCROLLED_WINDOW( ctxt->cmd_scroll_msg ), GTK_SHADOW_ETCHED_IN );
    ...
    gtk_box_pack_start( GTK_BOX( ctxt->cmd_vbox_msg ), GTK_WIDGET( ctxt->cmd_scroll_msg ), TRUE, TRUE, 4 );
    gtk_box_pack_start( GTK_BOX( vbox_frame ), GTK_WIDGET( ctxt->cmd_vbox_msg ), TRUE, TRUE, 0 );

    // open folder
    hbox = gtk_hbox_new( FALSE, 0 );
    label = gtk_label_new( _("Open in File Browser:") );
    gtk_misc_set_alignment( GTK_MISC ( label ), 0, 0.5 );
    ...
    str = g_strdup_printf( "%s  $fm_cmd_dir  %s", _("Command Dir"), dir_has_files( path ) ? "" : _("(no files)") );
    gtk_combo_box_text_append_text( GTK_COMBO_BOX_TEXT( ctxt->open_browser ), str );
    g_free( str );
    g_free( path );

    path = g_build_filename( xset_get_config_dir(), "mydat", rset->name, NULL );
    str = g_strdup_printf( "%s  $fm_cmd_data  %s", _("Data Dir"), dir_has_files( path ) ? "" : _("(no files)") );
    gtk_combo_box_text_append_text( GTK_COMBO_BOX_TEXT( ctxt->open_browser ), str );
    g_free( str );
    g_free( path );
    ...
    if ( set->tool > XSET_TOOL_CUSTOM )
        item_type_str = g_strdup_printf( "%s: %s", _("Built-In Toolbar Item"), xset_get_builtin_toolitem_label( set->tool ) );
    ...
    else if ( set->lock )
    {
        ...
        //g_signal_connect( G_OBJECT( ctxt->item_type ), "changed", G_CALLBACK( on_item_type_changed ), ctxt );
    }
    if ( item_type_str )
    {
    ...
    if ( set->lock || rset->menu_style == XSET_MENU_SUBMENU ||  rset->menu_style == XSET_MENU_SEP ||  set->tool > XSET_TOOL_CUSTOM )
    {
        gtk_widget_hide( gtk_notebook_get_nth_page( GTK_NOTEBOOK( ctxt->notebook ), 2 ) );
        gtk_widget_hide( gtk_notebook_get_nth_page( GTK_NOTEBOOK( ctxt->notebook ), 3 ) );
        gtk_widget_hide( ctxt->target_vbox );
    ...
    if ( rset->menu_style != XSET_MENU_SEP )
    {
        if ( set->menu_label )
            ...
            gtk_entry_set_text( GTK_ENTRY( ctxt->item_name ), xset_get_builtin_toolitem_label( set->tool ) );
    }
    else
        gtk_widget_set_sensitive( ctxt->item_name, FALSE );
    // key
    if ( rset->menu_style < XSET_MENU_SUBMENU ||  set->tool == XSET_TOOL_BACK_MENU ||  set->tool == XSET_TOOL_FWD_MENU )
    {
        XSet* keyset;
        if ( set->shared_key )
    ...
    gtk_widget_set_sensitive( ctxt->item_icon, rset->menu_style != XSET_MENU_RADIO &&  rset->menu_style != XSET_MENU_SEP );
                    // toolbar checkbox items have icon
                    //( rset->menu_style != XSET_MENU_CHECK || rset->tool ) );
    ...
    if ( set->tool )
    {
        ...
        //gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( ctxt->show_tool ), set->tool == XSET_B_TRUE );
    }
    //else
    ...
    // run
    enable_context( ctxt );
    if ( page && gtk_widget_get_visible( gtk_notebook_get_nth_page( GTK_NOTEBOOK( ctxt->notebook ), page ) ) )
