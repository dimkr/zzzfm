...
gboolean on_update_labels( FilePropertiesDialogData* data )
{
    ...
    {
        count_dir = g_strdup_printf( ngettext( "%d folder", "%d folders", data->total_count_dir ), data->total_count_dir );
        count = g_strdup_printf( ngettext( "%d file, %s", "%d files, %s", data->total_count ), data->total_count, count_dir );
        g_free( count_dir );
    }
    else
        count = g_strdup_printf( ngettext( "%d files", "%d files", data->total_count), data->total_count );
...
static void on_chmod_btn_toggled( GtkToggleButton* btn, FilePropertiesDialogData* data )
{
    ...
    g_signal_handlers_block_matched( btn, G_SIGNAL_MATCH_FUNC, 0, 0, NULL, on_chmod_btn_toggled, NULL );
    ...
    g_signal_handlers_unblock_matched( btn, G_SIGNAL_MATCH_FUNC, 0, 0, NULL, on_chmod_btn_toggled, NULL );
}
...
static void on_combo_change( GtkComboBox* combo, gpointer user_data ) {
    ...
    if( gtk_combo_box_get_active_iter(combo, &it) )
    {
        ...
        if( ! action )
        {
            ...
            action = (char *) ptk_choose_app_for_mime_type( GTK_WINDOW(parent), mime, FALSE, TRUE, TRUE, TRUE );
            if( action )
            {
                ...
                if( ! exist ) /* It didn't exist */
                {
                    ...
                    if( app )
                    {
                        ...
                        gtk_list_store_insert_with_values(
                                            GTK_LIST_STORE( model ), &it, 0, 0, icon, 1, vfs_app_desktop_get_disp_name(app), 2, action, -1 );
                        if( icon )
                            g_object_unref( icon );
...
GtkWidget* file_properties_dlg_new( GtkWindow* parent, const char* dir_path, GList* sel_files, int page ) {
...
*/
    ...
    if ( same_type )
    {
        ...
        file_type = g_strdup_printf( "%s\n%s", vfs_mime_type_get_description( mime ), vfs_mime_type_get_type( mime ) );
    ...
    if ( sel_files && sel_files->next )
    {
        ...
        for ( i = 0; i < N_CHMOD_ACTIONS; ++i )
        {
            ...
            g_signal_connect( G_OBJECT( data->chmod_btns[ i ] ), "toggled", G_CALLBACK( on_chmod_btn_toggled ), data );
        }
    } else {
        ...
        if ( ! vfs_file_info_is_dir( file ) )
        {
            ...
            sprintf( buf, _("%s  ( %lu bytes )"), buf2, ( guint64 ) vfs_file_info_get_blocks( file ) * 512 );
            gtk_label_set_text( data->size_on_disk_label, buf );
    ...
    if ( need_calc_size )
    {
        ...
        data->calc_size_thread = g_thread_create ( ( GThreadFunc ) calc_size, data, TRUE, NULL );
    ...
    g_signal_connect_swapped( gtk_builder_get_object(builder, "ok_button"), "clicked", G_CALLBACK(gtk_widget_destroy), dlg );
    g_signal_connect_swapped( gtk_builder_get_object(builder, "cancel_button"), "clicked", G_CALLBACK(gtk_widget_destroy), dlg );
    ...
    if ( data )
    {
        ...
        if ( response_id == GTK_RESPONSE_OK )
        {
            ...
            if ( ( new_mtime || new_atime ) && data->file_list )
            {
                ...
                for ( l = data->file_list; l; l = l->next )
                {
                    file_path = g_build_filename( data->dir_path, ((VFSFileInfo*)l->data)->name, NULL );
                ...
                if ( new_mtime )
                {
                    ...
                    cmd = g_strdup_printf( "touch --no-dereference --no-create -m -d %s%s", quoted_time, gstr->str );
                }
                if ( new_atime )
                {
                    quoted_time = bash_quote( new_atime );
                    quoted_path = cmd;  // temp str
                    cmd = g_strdup_printf( "%s%stouch --no-dereference --no-create -a -d %s%s", cmd ? cmd : "", cmd ? "\n" : "", quoted_time, gstr->str );
                    g_free( quoted_path );
                }
                g_free( quoted_time );
            ...
            if ( uid != -1 || gid != -1 || mod_change )
            {
                ...
                {
                    file = ( VFSFileInfo* ) l->data;
                    file_path = g_build_filename( data->dir_path, vfs_file_info_get_name( file ), NULL );
                ...
                for ( l = data->file_list; l; l = l->next )
                {
                    ...
                    if ( vfs_file_info_is_dir( file ) )
                    {
                        ask_recursive = gtk_message_dialog_new(
                                            GTK_WINDOW( data->dlg ), GTK_DIALOG_MODAL, GTK_MESSAGE_QUESTION, GTK_BUTTONS_YES_NO,
                                            _( "Do you want to recursively apply these changes to all files and sub-folders?" ) );
                        ptk_file_task_set_recursive( task, ( GTK_RESPONSE_YES == gtk_dialog_run( GTK_DIALOG( ask_recursive ) ) ) );
