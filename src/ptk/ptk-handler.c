...
const Handler handlers_net[]=
{
    ...
    */
    ...
    },
    {
        ...
        "smb mtab_fs=cifs",
        "",
        "UDEVIL_RESULT=\"$(udevil mount \"$fm_url\" | grep Mounted)\"\n[ -n \"$UDEVIL_RESULT\" ] && zzzfm -s set new_tab \"${UDEVIL_RESULT#* at }\"",
        TRUE,
        "udevil umount \"%a\"",
        FALSE,
        INFO_EXAMPLE,
...
char* ptk_handler_get_command( int mode, int cmd, XSet* handler_set ) {   /* if handler->disable, get const command, else get script path if exists */
    if ( !handler_set )
        return NULL;
    ...
    // name script
    char* str = g_strdup_printf( "/hand-%s-%s.sh", modes[mode], mode == HANDLER_MODE_ARC ?
...
char* ptk_handler_save_script( int mode, int cmd, XSet* handler_set, GtkTextView* view, const char* command ) {
...
gboolean ptk_handler_values_in_list( const char* list, GSList* values, char** msg ) {   /* test for the presence of values in list, using wildcards.
...
static gboolean value_in_list( const char* list, const char* value ) {   // this function must be FAST - is run multiple times on menu popup
...
void ptk_handler_add_new_default( int mode, const char* default_name, gboolean start ) {
...
void ptk_handler_import( int mode, GtkWidget* handler_dlg, XSet* set ) {    //  howdy bub     revisit this
    ...
    if ( g_strcmp0( xset_get_s( handler_conf_xset[mode] ), "" ) <= 0 )
    {
        ...
        gchar* new_handlers_list = g_strdup_printf( "%s %s", new_handler_xset->name, xset_get_s( handler_conf_xset[mode] ) );
        xset_set( handler_conf_xset[mode], "s", new_handlers_list );
        g_free( new_handlers_list );
    }

    // have handler dialog open?
    HandlerData* hnd = handler_dlg && GTK_IS_WIDGET( handler_dlg ) ?  (HandlerData*)g_object_get_data( G_OBJECT( handler_dlg ), "hnd" ) :   NULL;
    if ( !( hnd && hnd->dlg == handler_dlg && hnd->mode == mode ) )
    {
        ...
        if ( mode == HANDLER_MODE_ARC )
            ...
            return;  // failsafe
        msg = g_strdup_printf("The selected %s Handler file has been imported to the %s Handlers list.", mode_name, mode_name );
        GDK_THREADS_ENTER(); // due to dialog run causes low level thread lock
        xset_msg_dialog( NULL, GTK_MESSAGE_INFO, "Handler Imported", NULL, 0, msg, NULL, NULL );
        GDK_THREADS_LEAVE();
        g_free( msg );
        return;
    }
    ...
    char* dis_name = g_strdup_printf( "%s %s", new_handler_xset->menu_label, new_handler_xset->b == XSET_B_TRUE ?  "" :  disabled );
    gtk_list_store_set( GTK_LIST_STORE( hnd->list ), &iter, COL_XSET_NAME, new_handler_xset->name, COL_HANDLER_NAME, dis_name, -1 );
    g_free( dis_name );
...
static void config_load_handler_settings( XSet* handler_xset, gchar* handler_xset_name, const Handler* handler, HandlerData* hnd ) {
    ...
    gtk_widget_set_sensitive( GTK_WIDGET( hnd->btn_defaults0 ), g_str_has_prefix( handler_xset->name, handler_def_prefix[hnd->mode] ) );
    ...
    int start;
    gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( hnd->chkbtn_handler_enabled ), handler_xset->b == XSET_B_TRUE );

    gtk_entry_set_text( GTK_ENTRY( hnd->entry_handler_name ),  handler_xset->menu_label ?  handler_xset->menu_label : "" );
    gtk_entry_set_text( GTK_ENTRY( hnd->entry_handler_mime ),  handler_xset->s ?  handler_xset->s : "" );
    gtk_entry_set_text( GTK_ENTRY( hnd->entry_handler_extension ),   handler_xset->x ?  handler_xset->x : "" );
    if ( hnd->entry_handler_icon )
        gtk_entry_set_text( GTK_ENTRY( hnd->entry_handler_icon ),  handler_xset->icon ?  handler_xset->icon : "" );

    if ( handler )
    {
        // load commands from const handler
        ptk_handler_load_text_view( GTK_TEXT_VIEW( hnd->view_handler_compress ), handler->compress_cmd );
        if ( hnd->mode != HANDLER_MODE_FILE )
        {
            ptk_handler_load_text_view( GTK_TEXT_VIEW( hnd->view_handler_extract ), handler->extract_cmd );
            ptk_handler_load_text_view( GTK_TEXT_VIEW( hnd->view_handler_list ), handler->list_cmd );
        }
    } else {
        char* err_msg = ptk_handler_load_script( hnd->mode, HANDLER_COMPRESS, handler_xset,
                            GTK_TEXT_VIEW( hnd->view_handler_compress ), NULL );
        if ( hnd->mode != HANDLER_MODE_FILE )
        {
            if ( !err_msg )
                err_msg = ptk_handler_load_script( hnd->mode, HANDLER_EXTRACT, handler_xset,
                                GTK_TEXT_VIEW( hnd->view_handler_extract ), NULL );
            if ( !err_msg )
                err_msg = ptk_handler_load_script( hnd->mode, HANDLER_LIST, handler_xset,
                                GTK_TEXT_VIEW( hnd->view_handler_list ), NULL );
        }
        if ( err_msg )
        {
            xset_msg_dialog( GTK_WIDGET( hnd->dlg ), GTK_MESSAGE_ERROR, _("Error Loading Handler"), NULL, 0, err_msg, NULL, NULL );
            g_free( err_msg );
        }
    }
...
static void populate_archive_handlers( HandlerData* hnd, XSet* def_handler_set ) {
     ...
     * present */
    if ( i > 0 &&
        !gtk_tree_selection_get_selected( GTK_TREE_SELECTION( selection ), NULL, NULL ) )
    {
...
static void on_configure_drag_end( GtkWidget* widget, GdkDragContext* drag_context, HandlerData* hnd ) {
    ...
    if (!gtk_tree_model_get_iter_first( GTK_TREE_MODEL( hnd->list ), &iter ))
    {
        ...
        g_warning("Drag'n'drop end event detected, but unable to get an iterator to the start of the model!");
        return;
    }
...
static void on_configure_button_press( GtkButton* widget, HandlerData* hnd ) {
    ...
    const gchar* handler_name = gtk_entry_get_text( GTK_ENTRY ( hnd->entry_handler_name ) );
    const gchar* handler_mime = gtk_entry_get_text( GTK_ENTRY ( hnd->entry_handler_mime ) );
    const gchar* handler_extension = gtk_entry_get_text( GTK_ENTRY ( hnd->entry_handler_extension ) );
    ...
    if ( gtk_tree_selection_get_selected( GTK_TREE_SELECTION( selection ), &model, &it ) )
    {
        ...
        gtk_tree_model_get( model, &it, COL_XSET_NAME, &xset_name, COL_HANDLER_NAME, &handler_name_from_model, -1 );
    ...
    if ( GTK_WIDGET( widget ) == GTK_WIDGET( hnd->btn_add ) )
    {
        ...
        new_handler_xset->b = gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( hnd->chkbtn_handler_enabled )
        ...
        new_handler_xset->keep_terminal = handler_extract_term ?
                                                XSET_B_TRUE : XSET_B_UNSET;
        err_msg = ptk_handler_save_script( hnd->mode, HANDLER_COMPRESS, new_handler_xset, GTK_TEXT_VIEW( hnd->view_handler_compress ), NULL );
        if ( hnd->mode == HANDLER_MODE_FILE )
        {
            if ( handler_icon )
            ...
            new_handler_xset->scroll_lock = handler_list_term ?
                                                    XSET_B_TRUE : XSET_B_UNSET;
            if ( !err_msg )
                err_msg = ptk_handler_save_script( hnd->mode, HANDLER_EXTRACT, new_handler_xset, GTK_TEXT_VIEW( hnd->view_handler_extract ), NULL );
            if ( !err_msg )
                err_msg = ptk_handler_save_script( hnd->mode, HANDLER_LIST, new_handler_xset, GTK_TEXT_VIEW( hnd->view_handler_list ), NULL );
        }
    ...
    else if ( GTK_WIDGET( widget ) == GTK_WIDGET( hnd->btn_apply ) )
    {
        ...
        gboolean handler_enabled = gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( hnd->chkbtn_handler_enabled ) );
        ...
        if (g_strcmp0( handler_name_from_model, handler_name ) != 0)
        {
            ...
            const char* disabled = hnd->mode == HANDLER_MODE_FILE ?
                                                _("(optional)") :
                                                _("(disabled)");
            char* dis_name = g_strdup_printf( "%s %s", handler_name, handler_enabled ? "" : disabled );
            gtk_list_store_set( GTK_LIST_STORE( model ), &it, COL_XSET_NAME, xset_name, COL_HANDLER_NAME, dis_name, -1 );
            g_free( dis_name );
        }
        ...
        {
            err_msg = ptk_handler_save_script( hnd->mode, HANDLER_COMPRESS, handler_xset, GTK_TEXT_VIEW( hnd->view_handler_compress ), NULL );
        }
        if ( hnd->mode == HANDLER_MODE_FILE )
        {
            if ( handler_icon )
            ...
            {
                str = ptk_handler_save_script( hnd->mode, HANDLER_EXTRACT, handler_xset, GTK_TEXT_VIEW( hnd->view_handler_extract ), NULL );
                if ( str )
                {
                    if ( !err_msg )
                        err_msg = str;
            ...
            {
                str = ptk_handler_save_script( hnd->mode, HANDLER_LIST, handler_xset, GTK_TEXT_VIEW( hnd->view_handler_list ), NULL );
                if ( str )
                {
                    if ( !err_msg )
                        err_msg = str;
    ...
    else if ( GTK_WIDGET( widget ) == GTK_WIDGET( hnd->btn_remove ) )
    {
        ...
        if ( xset_msg_dialog( hnd->dlg, GTK_MESSAGE_WARNING, _("Confirm Remove"), NULL,
                          GTK_BUTTONS_YES_NO, _("Permanently remove the selected handler?"), NULL, NULL ) != GTK_RESPONSE_YES )
            goto _clean_exit;
        ...
        if ( archive_handlers )
        {
            for (i = 0; archive_handlers[i] != NULL; ++i)
            {
                ...
                {
                    // Debug code
                    //g_message("archive_handlers[i]: %s\nxset_name: %s",   archive_handlers[i], xset_name);
                    ...
                    if (g_strcmp0( new_archive_handlers_s, "" ) == 0)
                    {
                        ...
                        new_archive_handlers_s = g_strdup_printf( "%s %s", new_archive_handlers_s, archive_handlers[i] );
                    }
        ...
        if (g_strcmp0( new_archive_handlers_s, "" ) == 0)
        {
            ...
            //  Still remaining handlers - selecting the first one,  otherwise nothing is now selected
            GtkTreePath *new_path = gtk_tree_path_new_first();
            gtk_tree_selection_select_path( GTK_TREE_SELECTION( selection ), new_path );
            gtk_tree_path_free( new_path );
        }
    ...
    if ( err_msg )
    {
        xset_msg_dialog( GTK_WIDGET( hnd->dlg ), GTK_MESSAGE_ERROR, _("Error Saving Handler"), NULL, 0, err_msg, NULL, NULL );
        g_free( err_msg );
    }
...
static void on_configure_changed( GtkTreeSelection* selection, HandlerData* hnd ) {
    /* This event is triggered when the selected row is changed or no row is selected at all */
     ...
     * of the text... */
    /*GtkWidget* entry_handler_name = (GtkWidget*)g_object_get_data( G_OBJECT( dlg ), "entry_handler_name" );
...
static gboolean on_handlers_key_press( GtkWidget* widget, GdkEventKey* evt, HandlerData* hnd ) {
    // Current handler hasn't been changed?
    if ( !hnd->changed /* was !gtk_widget_get_sensitive( hnd->btn_apply )*/ )
        return FALSE;

    if ( xset_msg_dialog( hnd->dlg, GTK_MESSAGE_QUESTION, _("Apply Changes ?"), NULL, GTK_BUTTONS_YES_NO,
                          _("Apply changes to the current handler?"), NULL, NULL ) == GTK_RESPONSE_YES )
...
static gboolean on_handlers_button_press( GtkWidget* view, GdkEventButton* evt, HandlerData* hnd ) {
    ...
    if ( gtk_widget_get_sensitive( hnd->btn_apply ) )
    {
        ...
        if ( xset_msg_dialog( hnd->dlg, GTK_MESSAGE_QUESTION,
                              _("Apply Changes ?"), NULL, GTK_BUTTONS_YES_NO,
                              _("Apply changes to the current handler?"), NULL, NULL ) == GTK_RESPONSE_YES )
            on_configure_button_press( GTK_BUTTON( hnd->btn_apply ), hnd );
...
static void on_configure_row_activated( GtkTreeView* view, GtkTreePath* tree_path, GtkTreeViewColumn* col, HandlerData* hnd )
{
    // This event is triggered when the selected row is changed by the mouse
...
static gboolean validate_archive_handler( HandlerData* hnd ) {
    ...
    if (g_strcmp0( handler_name, "" ) <= 0)
    {
        ...
        xset_msg_dialog( GTK_WIDGET( hnd->dlg ), GTK_MESSAGE_WARNING, _(dialog_titles[hnd->mode]), NULL, FALSE, _("Please enter a valid handler name."), NULL, NULL );
    ...
    {
        xset_msg_dialog( GTK_WIDGET( hnd->dlg ), GTK_MESSAGE_WARNING, _(dialog_titles[hnd->mode]), NULL, FALSE, _("Please enter a valid MIME Type or Pathname "
                            "pattern."), NULL, NULL );
     ...
     * commands, empty commands are allowed but if something is given,  relevant substitution characters should be in place */
    ...
    {
        //  ensure all substitution characters are in place ~~ not mandatory to only have ONE of the particular type
        if (
            (
    ...
    {
        //   Not all substitution characters are in place - warning user and exiting.
        //   Note that the created dialog does not have an icon set
        xset_msg_dialog( GTK_WIDGET( hnd->dlg ), GTK_MESSAGE_WARNING, _(dialog_titles[hnd->mode]), NULL, FALSE,
                            _("The following "
...
void on_textview_font_change( GtkMenuItem* item, HandlerData* hnd ) {
    ...
    PangoFontDescription* font_desc = fontname ?  pango_font_description_from_string( fontname ) : NULL;
...
static gboolean on_textview_keypress ( GtkWidget *widget, GdkEventKey *event, HandlerData* hnd ) {   // also used on dlg keypress
...
void on_entry_text_insert( GtkEntryBuffer* buffer, guint position, gchar* chars, guint n_chars, HandlerData* hnd ) {
    if ( !hnd->changed )
    {
        hnd->changed = TRUE;
        gtk_widget_set_sensitive( hnd->btn_apply, gtk_widget_get_sensitive( hnd->btn_remove ) );
    }
    if ( hnd->entry_handler_icon &&  gtk_entry_get_buffer( GTK_ENTRY( hnd->entry_handler_icon ) ) ==  buffer )
    {
        // update icon of icon choose button
        const char* icon = gtk_entry_get_text( GTK_ENTRY( hnd->entry_handler_icon ) );
        gtk_button_set_image( GTK_BUTTON( hnd->icon_choose_btn ), xset_get_image( icon && icon[0] ? icon :  GTK_STOCK_OPEN, GTK_ICON_SIZE_BUTTON ) );
    }
}

void on_entry_text_delete( GtkEntryBuffer* buffer, guint position, guint n_chars, HandlerData* hnd ) {
...
void on_option_cb( GtkMenuItem* item, HandlerData* hnd ) {
    ...
    if ( job == HANDLER_JOB_IMPORT_FILE )
    {
// howdy bub            xref   docs//eggcellllllleratorKeys//commandline+socketRemoteCommands
/*
        // get file path
        ...
        file = xset_file_dialog( GTK_WIDGET( hnd->dlg ), GTK_FILE_CHOOSER_ACTION_OPEN, "Choose Handler Plugin File", folder, NULL );
        if ( !file )
            return;
        if ( save->s )
            g_free( save->s );
        ...
        // Get URL
        file = NULL;
        // howdy bub                   wget bullshit   return unconditionally    TODO  nixme
      //if ( !xset_text_dialog( GTK_WIDGET( hnd->dlg ), "Enter Handler Plugin URL", NULL, FALSE, "Enter zzzFM Handler Plugin URL:\n\n(wget will be used to download the handler plugin file)", NULL, NULL, &file, NULL, FALSE, "#handlers-opt-impu" ) || !file || file[0] == '\0' )
*/
            return;
    }


    // Make Plugin Dir      howdy bub
    const char* user_tmp = xset_get_user_tmp_dir();
    if ( !user_tmp )
    {
        xset_msg_dialog( GTK_WIDGET( hnd->dlg ), GTK_MESSAGE_ERROR, _("Error Creating Temp Directory"), NULL, 0, _("Unable to create temporary directory"), NULL, NULL );
        g_free( file );
        return;
    }
    char* hex8;
    ...
    // Install plugin           howdy bub     recheck this (when/how is it reached?)
    install_plugin_file( NULL, hnd->dlg, file, folder, job == HANDLER_JOB_IMPORT_FILE ? 0 : 1, PLUGIN_JOB_COPY, NULL );
    g_free( file );
    g_free( folder );
}
                                      ...
                                      HandlerData* hnd )
{
    ...
    if ( image )
        gtk_image_menu_item_set_image ( GTK_IMAGE_MENU_ITEM( item ), xset_get_image( image, GTK_ICON_SIZE_MENU ) );
    gtk_container_add ( GTK_CONTAINER ( popup ), item );
    g_signal_connect( G_OBJECT( item ), "activate", G_CALLBACK( on_option_cb ), ( gpointer ) hnd );
...
static void on_options_button_clicked( GtkWidget* btn, HandlerData* hnd ) {
    ...
    GtkTreeSelection* selection = gtk_tree_view_get_selection( GTK_TREE_VIEW( hnd->view_handlers ) );
    gboolean handler_selected = gtk_tree_selection_get_selected( selection, NULL, NULL );
    ...
    if ( !btn )
    {
        ...
        item = add_popup_menuitem( popup, accel_group, _("_Remove"), GTK_STOCK_REMOVE, HANDLER_JOB_REMOVE, hnd );
        gtk_widget_set_sensitive( item, handler_selected );
    }

// howdy
//  item = add_popup_menuitem( popup, accel_group, "_Export", GTK_STOCK_SAVE, HANDLER_JOB_EXPORT, hnd );
//  gtk_widget_set_sensitive( item, handler_selected );

// howdy
//  add_popup_menuitem( popup, accel_group, "Import _File", GTK_STOCK_ADD, HANDLER_JOB_IMPORT_FILE, hnd );
//  add_popup_menuitem( popup, accel_group, "Import _URL", GTK_STOCK_NETWORK, HANDLER_JOB_IMPORT_URL, hnd );
    add_popup_menuitem( popup, accel_group, _("Restore _Default Handlers"), GTK_STOCK_REVERT_TO_SAVED, HANDLER_JOB_RESTORE_ALL, hnd );
    if ( btn )
    {
        ...
        else if ( hnd->mode == HANDLER_MODE_FS )
        {
            ...
            xset_context_new();
            gtk_container_add ( GTK_CONTAINER ( popup ), gtk_separator_menu_item_new() );
            xset_add_menuitem( hnd->desktop, hnd->browser, popup, accel_group, xset_get( "dev_mount_options" ) );
        }
    }
...
void ptk_handler_show_config( int mode, DesktopWindow* desktop, PtkFileBrowser* file_browser, XSet* def_handler_set ) {
    ...
    gtk_button_set_focus_on_click(
                                    GTK_BUTTON(
                                        gtk_dialog_add_button(
                                            GTK_DIALOG( hnd->dlg ), GTK_STOCK_HELP, GTK_RESPONSE_HELP
                                        )
                                    ), FALSE );

    // Adding standard buttons and saving references in the dialog
    // 'Restore defaults' button has custom text but a stock image
    hnd->btn_defaults = gtk_dialog_add_button( GTK_DIALOG( hnd->dlg ), _("Opt_ions"), GTK_RESPONSE_NONE );
    ...
    g_signal_connect( G_OBJECT( hnd->btn_defaults ), "clicked", G_CALLBACK( on_options_button_clicked ), hnd );

    hnd->btn_defaults0 = gtk_dialog_add_button( GTK_DIALOG( hnd->dlg ), _("Defa_ults"), GTK_RESPONSE_NO );
    GtkWidget* btn_defaults_image0 = xset_get_image( "GTK_STOCK_REVERT_TO_SAVED", GTK_ICON_SIZE_BUTTON );
    ...
    hnd->btn_cancel = gtk_dialog_add_button( GTK_DIALOG( hnd->dlg ), GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL );
    ...
    // widget is used for file lists, where double-clicking is the norm for doing an action)
...
/*igcr probably doesn't need to be single click, as you're not using row activation, only selection changed? */
    ...
    g_signal_connect( G_OBJECT( hnd->view_handlers ), "drag-end", G_CALLBACK( on_configure_drag_end ), hnd );
//  g_signal_connect( G_OBJECT( hnd->view_handlers ), "row-activated", G_CALLBACK( on_configure_row_activated ), hnd );
    g_signal_connect( G_OBJECT( gtk_tree_view_get_selection( GTK_TREE_VIEW( hnd->view_handlers ) ) ), "changed", G_CALLBACK( on_configure_changed ), hnd );
    g_signal_connect ( hnd->view_handlers, "button-press-event", G_CALLBACK ( on_handlers_button_press ), hnd );
    ...
    // Set column to take all available space - false by default     howdy   somewhere related to this is a resizing bug
    ...
    gtk_button_set_image( GTK_BUTTON( hnd->btn_remove ), xset_get_image( "GTK_STOCK_REMOVE", GTK_ICON_SIZE_BUTTON ) );
    gtk_button_set_focus_on_click( GTK_BUTTON( hnd->btn_remove ), FALSE );
    gtk_widget_set_sensitive( hnd->btn_remove, FALSE );
    g_signal_connect( G_OBJECT( hnd->btn_remove ), "clicked", G_CALLBACK( on_configure_button_press ), hnd );

    hnd->btn_add = gtk_button_new_with_mnemonic( _("A_dd") );
    gtk_button_set_image( GTK_BUTTON( hnd->btn_add ), xset_get_image( "GTK_STOCK_ADD", GTK_ICON_SIZE_BUTTON ) );
    ...
    gtk_button_set_image( GTK_BUTTON( hnd->btn_apply ), xset_get_image( "GTK_STOCK_APPLY", GTK_ICON_SIZE_BUTTON ) );
    gtk_button_set_focus_on_click( GTK_BUTTON( hnd->btn_apply ), FALSE );
    gtk_widget_set_sensitive( hnd->btn_apply, FALSE );
    g_signal_connect( G_OBJECT( hnd->btn_apply ), "clicked", G_CALLBACK( on_configure_button_press ), hnd );

    hnd->btn_up = gtk_button_new_with_mnemonic( _("U_p") );
    gtk_button_set_image( GTK_BUTTON( hnd->btn_up ), xset_get_image( "GTK_STOCK_GO_UP", GTK_ICON_SIZE_BUTTON ) );
    ...
    gtk_button_set_image( GTK_BUTTON( hnd->btn_down ), xset_get_image( "GTK_STOCK_GO_DOWN", GTK_ICON_SIZE_BUTTON ) );
    gtk_button_set_focus_on_click( GTK_BUTTON( hnd->btn_down ), FALSE );
    gtk_widget_set_sensitive( hnd->btn_down, FALSE );
    g_signal_connect( G_OBJECT( hnd->btn_down ), "clicked", G_CALLBACK( on_configure_button_press ), hnd );

    // Generating right-hand side of dialog
    hnd->chkbtn_handler_enabled = gtk_check_button_new_with_mnemonic( mode == HANDLER_MODE_FILE ?
                                            _("Ena_ble as a default opener") :  _("Ena_ble Handler") );
    gtk_button_set_focus_on_click( GTK_BUTTON( hnd->chkbtn_handler_enabled ), FALSE );
    g_signal_connect( G_OBJECT( hnd->chkbtn_handler_enabled ), "toggled", G_CALLBACK ( on_configure_handler_enabled_check ), hnd );
    GtkWidget* lbl_handler_name = gtk_label_new( NULL );
    gtk_label_set_markup_with_mnemonic( GTK_LABEL( lbl_handler_name ), _("_Name:") ), gtk_misc_set_alignment( GTK_MISC( lbl_handler_name ), 0, 0.8 );
    GtkWidget* lbl_handler_mime = gtk_label_new( NULL );
    gtk_label_set_markup_with_mnemonic( GTK_LABEL( lbl_handler_mime ), mode == HANDLER_MODE_ARC || mode == HANDLER_MODE_FILE ?
                                        _("MIM_E Type:") :    _("Whit_elist:") );
    gtk_misc_set_alignment( GTK_MISC( lbl_handler_mime ), 0, 0.8 );
    GtkWidget* lbl_handler_extension = gtk_label_new( NULL );
    gtk_label_set_markup_with_mnemonic( GTK_LABEL( lbl_handler_extension ), mode == HANDLER_MODE_ARC ||
                                        mode == HANDLER_MODE_FILE ?  _("P_athname:") :  _("Bl_acklist:") );
    ...
    gtk_label_set_markup_with_mnemonic( GTK_LABEL( lbl_handler_extract ), mode == HANDLER_MODE_ARC ?
                                        _("<b>Ex_tract:</b>") :
                                        _("<b>Unmoun_t:</b>") );
    gtk_misc_set_alignment( GTK_MISC( lbl_handler_extract ), 0, 1.0 );
    GtkWidget* lbl_handler_list = gtk_label_new( NULL );
    gtk_label_set_markup_with_mnemonic( GTK_LABEL( lbl_handler_list ), mode == HANDLER_MODE_ARC ?
                                        _("<b>Li_st:</b>") :
                                        _("<b>Propertie_s:</b>") );
    ...
    if ( mode == HANDLER_MODE_FILE )
    {
        ...
        gtk_button_set_image( GTK_BUTTON( hnd->icon_choose_btn ), xset_get_image( GTK_STOCK_OPEN, GTK_ICON_SIZE_BUTTON ) );
    ...
    g_signal_connect( G_OBJECT( gtk_entry_get_buffer( GTK_ENTRY( hnd->entry_handler_name ) ) ), "inserted-text", G_CALLBACK( on_entry_text_insert ), hnd );
    g_signal_connect( G_OBJECT( gtk_entry_get_buffer( GTK_ENTRY( hnd->entry_handler_name ) ) ), "deleted-text", G_CALLBACK( on_entry_text_delete ), hnd );
    g_signal_connect( G_OBJECT( gtk_entry_get_buffer( GTK_ENTRY( hnd->entry_handler_mime ) ) ), "inserted-text", G_CALLBACK( on_entry_text_insert ), hnd );
    g_signal_connect( G_OBJECT( gtk_entry_get_buffer( GTK_ENTRY( hnd->entry_handler_mime ) ) ), "deleted-text", G_CALLBACK( on_entry_text_delete ), hnd );
    g_signal_connect( G_OBJECT( gtk_entry_get_buffer( GTK_ENTRY( hnd->entry_handler_extension ) ) ), "inserted-text", G_CALLBACK( on_entry_text_insert ), hnd );
    g_signal_connect( G_OBJECT( gtk_entry_get_buffer( GTK_ENTRY( hnd->entry_handler_extension ) ) ), "deleted-text", G_CALLBACK( on_entry_text_delete ), hnd );
    if ( hnd->entry_handler_icon )
    {
        g_signal_connect(
    ...
    gtk_scrolled_window_set_policy ( GTK_SCROLLED_WINDOW ( view_handler_compress_scroll ), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC );
    gtk_container_add( GTK_CONTAINER( view_handler_compress_scroll ), hnd->view_handler_compress );
    g_signal_connect( G_OBJECT( hnd->view_handler_compress ), "key-press-event", G_CALLBACK( on_textview_keypress ), hnd );
    hnd->buf_handler_compress = gtk_text_view_get_buffer( GTK_TEXT_VIEW( hnd->view_handler_compress ) );
    g_signal_connect( G_OBJECT( hnd->buf_handler_compress ), "changed", G_CALLBACK( on_textview_buffer_changed ), hnd );
    ...
    gtk_scrolled_window_set_policy ( GTK_SCROLLED_WINDOW ( view_handler_extract_scroll ), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC );
    gtk_container_add( GTK_CONTAINER( view_handler_extract_scroll ), hnd->view_handler_extract );
    g_signal_connect( G_OBJECT( hnd->view_handler_extract ), "key-press-event", G_CALLBACK( on_textview_keypress ), hnd );
    hnd->buf_handler_extract = gtk_text_view_get_buffer( GTK_TEXT_VIEW( hnd->view_handler_extract ) );
    g_signal_connect( G_OBJECT( hnd->buf_handler_extract ), "changed", G_CALLBACK( on_textview_buffer_changed ), hnd );
    ...
    gtk_scrolled_window_set_policy ( GTK_SCROLLED_WINDOW ( view_handler_list_scroll ), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC );
    gtk_container_add( GTK_CONTAINER( view_handler_list_scroll ), hnd->view_handler_list );
    g_signal_connect( G_OBJECT( hnd->view_handler_list ), "key-press-event", G_CALLBACK( on_textview_keypress ), hnd );
    hnd->buf_handler_list = gtk_text_view_get_buffer( GTK_TEXT_VIEW( hnd->view_handler_list ) );
    g_signal_connect( G_OBJECT( hnd->buf_handler_list ), "changed", G_CALLBACK( on_textview_buffer_changed ), hnd );

    // set fonts
    on_textview_font_change( NULL, hnd );
    // set textview popup menu event handlers
    g_signal_connect_after( G_OBJECT( hnd->view_handler_compress ), "populate-popup", G_CALLBACK( on_textview_popup ), hnd );
    g_signal_connect_after( G_OBJECT( hnd->view_handler_extract ), "populate-popup", G_CALLBACK( on_textview_popup ), hnd );
    g_signal_connect_after( G_OBJECT( hnd->view_handler_list ), "populate-popup", G_CALLBACK( on_textview_popup ), hnd );
    ...
    hnd->chkbtn_handler_extract_term =
                gtk_check_button_new_with_label( mode == HANDLER_MODE_FILE ? _("Run As Task") :  _("Run In Terminal") );
    hnd->chkbtn_handler_list_term =
                gtk_check_button_new_with_label( _("Run In Terminal") );
    gtk_button_set_focus_on_click( GTK_BUTTON( hnd->chkbtn_handler_compress_term ), FALSE );
    g_signal_connect( G_OBJECT( hnd->chkbtn_handler_compress_term ), "toggled", G_CALLBACK( on_terminal_toggled ), hnd );
    gtk_button_set_focus_on_click( GTK_BUTTON( hnd->chkbtn_handler_extract_term ), FALSE );
    g_signal_connect( G_OBJECT( hnd->chkbtn_handler_extract_term ), "toggled", G_CALLBACK( on_terminal_toggled ), hnd );
    gtk_button_set_focus_on_click( GTK_BUTTON( hnd->chkbtn_handler_list_term ), FALSE );
    g_signal_connect( G_OBJECT( hnd->chkbtn_handler_list_term ), "toggled", G_CALLBACK( on_terminal_toggled ), hnd );
    ...
    gtk_table_attach( GTK_TABLE( tbl_settings ), GTK_WIDGET( lbl_handler_name ), 0, 1, 0, 1, GTK_FILL, GTK_FILL, 0, 0 );
    gtk_table_attach( GTK_TABLE( tbl_settings ), GTK_WIDGET( hnd->entry_handler_name ), 1, 4, 0, 1, GTK_EXPAND | GTK_FILL, GTK_FILL, 0, 0 );
    gtk_table_attach( GTK_TABLE( tbl_settings ), GTK_WIDGET( lbl_handler_mime ), 0, 1, 1, 2, GTK_FILL, GTK_FILL, 0, 0 );
    gtk_table_attach( GTK_TABLE( tbl_settings ), GTK_WIDGET( hnd->entry_handler_mime ), 1, 4, 1, 2, GTK_EXPAND | GTK_FILL, GTK_FILL, 0, 0 );
    gtk_table_attach( GTK_TABLE( tbl_settings ), GTK_WIDGET( lbl_handler_extension ), 0, 1, 2, 3, GTK_FILL, GTK_FILL, 0, 0 );
    gtk_table_attach( GTK_TABLE( tbl_settings ), GTK_WIDGET( hnd->entry_handler_extension ), 1, 4, 2, 3, GTK_EXPAND | GTK_FILL, GTK_FILL, 0, 0 );
    if ( mode == HANDLER_MODE_FILE )
    {
        gtk_table_attach( GTK_TABLE( tbl_settings ), GTK_WIDGET( lbl_handler_icon ), 0, 1, 3, 4, GTK_FILL, GTK_FILL, 0, 0 );
        ...
        gtk_table_attach( GTK_TABLE( tbl_settings ), GTK_WIDGET( hbox_icon ), 1, 4, 3, 4, GTK_EXPAND | GTK_FILL, GTK_FILL, 0, 0 );
    }
    ...
    gtk_box_pack_start( GTK_BOX( hbox_compress_header ), GTK_WIDGET( lbl_handler_compress ), TRUE, TRUE, 4 );
    if ( mode == HANDLER_MODE_FILE )
        // for file handlers, extract_term is used for Run As Task
        gtk_box_pack_start( GTK_BOX( hbox_compress_header ), hnd->chkbtn_handler_extract_term, FALSE, TRUE, 4 );
    gtk_box_pack_start( GTK_BOX( hbox_compress_header ), hnd->chkbtn_handler_compress_term, FALSE, TRUE, 4 );
    gtk_box_pack_end( GTK_BOX( hbox_compress_header ), GTK_WIDGET( lbl_edit0 ), FALSE, FALSE, 4 );
    gtk_box_pack_start( GTK_BOX( vbox_settings ), GTK_WIDGET( view_handler_compress_scroll ), TRUE, TRUE, 4 );

    gtk_box_pack_start( GTK_BOX( vbox_settings ), GTK_WIDGET( hbox_extract_header ), FALSE, FALSE, 4 );
    gtk_box_pack_start( GTK_BOX( hbox_extract_header ), GTK_WIDGET( lbl_handler_extract ), TRUE, TRUE, 4 );
    if ( mode != HANDLER_MODE_FILE )
        gtk_box_pack_start( GTK_BOX( hbox_extract_header ), hnd->chkbtn_handler_extract_term, FALSE, TRUE, 4 );
    gtk_box_pack_end( GTK_BOX( hbox_extract_header ), GTK_WIDGET( lbl_edit1 ), FALSE, FALSE, 4 );
    gtk_box_pack_start( GTK_BOX( vbox_settings ), GTK_WIDGET( view_handler_extract_scroll ), TRUE, TRUE, 4 );
