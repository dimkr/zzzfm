...
void ptk_menu_add_items_from_data( GtkWidget* menu, PtkMenuItemEntry* entries, gpointer cb_data, GtkAccelGroup* accel_group )
{
...
void ptk_show_error(GtkWindow* parent, const char* title, const char* message )
{
    ...
    GtkWidget* dlg = gtk_message_dialog_new(parent, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, msg, NULL);
    g_free( msg );
    if( title )
...
// GTK3 no longer accepts NULL for default keymap
                                              ...
                                              NULL,
#endif
                                              event->hardware_keycode, (GdkModifierType)event->state, event->group, NULL, NULL, &level, NULL )
